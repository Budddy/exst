    ...
    namespace Test {
        ...
        public:
            ...
            void testEnumerate() {
                ...
                newMin = buildAndAttach(MinimizeBuilder()
                                                .addRule(aMin)
                                                .addRule(bMin), MinimizeMode_t::enumerate, bound, 2);
        ...
        private:
            ...
            DefaultMinimize *createMin(SharedContext &ctx, Solver &s, SharedMinimizeData *data,
                                       MinimizeMode_t::BBOption param = MinimizeMode_t::bb_step_def) {
                ctx.endInit();
                return static_cast<DefaultMinimize *>(data->attach(s, MinimizeMode_t::opt_bb, param));
            }
            DefaultMinimize *buildAndAttach(MinimizeBuilder &x, MinimizeMode m = MinimizeMode_t::optimize,
                                            const wsum_t *b = 0, uint32 bs = 0) {
