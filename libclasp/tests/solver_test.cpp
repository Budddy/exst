    ...
    namespace Test {
        ...
        struct TestingConstraint : public LearntConstraint {
            TestingConstraint(bool *del = 0, ConstraintType t = Constraint_t::static_constraint)
                    : type_(t), propagates(0), undos(0), sat(false), keepWatch(true), setConflict(false),
                      deleted(del) { }
        ...
        struct TestingPostProp : public PostPropagator {
            explicit TestingPostProp(bool cfl, uint32 p = PostPropagator::priority_class_simple) : props(0), resets(0),
                                                                                                   inits(0), prio(p),
                                                                                                   conflict(cfl),
                                                                                                   deleteOnProp(
                                                                                                           false) { }
        ...
        public:
            ...
            void testAssertImmediate() {
                ...
                CPPUNIT_ASSERT(whyZ.type() == Antecedent::ternary_constraint && whyZ.firstLiteral() == ~a &&
                               whyZ.secondLiteral() == b);
                ...
                CPPUNIT_ASSERT(whyX.type() == Antecedent::ternary_constraint && whyX.firstLiteral() == f &&
                               whyX.secondLiteral() == z);
            }
            ...
            void testResolveConflict() {
                Literal x1 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x2 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x3 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x4 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x5 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x6 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x7 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x8 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x9 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x10 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x11 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x12 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x13 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x14 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x15 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x16 = posLit(ctx.addVar(Var_t::atom_var));
            ...
            void testResolveConflictBounded() {
                ...
                Literal x16 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x17 = posLit(ctx.addVar(Var_t::atom_var));
                Literal x18 = posLit(ctx.addVar(Var_t::atom_var));
                Solver &s = ctx.startAddConstraints();
                ClauseCreator cl(&s);
                cl.start().add(~x11).add(x12).end();
                cl.start().add(x1).add(~x12).add(~x13).end();
            ...
            void testStats() {
                ...
                p1.vars = 100;
                p2.vars = 150;
                p1.vars_eliminated = 20;
                p2.vars_eliminated = 30;
                p1.constraints = 150;
                p2.constraints = 150;
                p1.constraints_binary = 0;
                p2.constraints_binary = 100;
                p1.constraints_ternary = 100;
                p2.constraints_ternary = 0;
                p1.diff(p2);
            ...
            void testLearntShortAreDistributed() {
                ...
                struct Dummy : public Distributor {
                    Dummy() : Distributor(Policy(UINT32_MAX, UINT32_MAX, UINT32_MAX)), unary(0), binary(0),
                              ternary(0) { }
