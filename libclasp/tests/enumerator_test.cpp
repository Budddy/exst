    ...
    namespace Test {
        ...
        public:
            ...
            void testProjectBug() {
                ...
                Solver &solver = *ctx.master();
                builder.start(ctx, LogicProgram::AspOptions().noEq().noScc())
                        .setAtomName(1, "x").setAtomName(2, "y").setAtomName(3, "z").setAtomName(4, "_p").setAtomName(5,
                                                                                                                      "_q").setAtomName(
                                6, "_r")
            ...
            void testTerminateRemovesWatches() {
                ...
                uint32 numW = solver.numWatches(index[1].lit) + solver.numWatches(index[2].lit) +
                              solver.numWatches(index[3].lit) + solver.numWatches(index[4].lit);
                CPPUNIT_ASSERT(numW > 0);
                ctx.detach(solver);
                numW = solver.numWatches(index[1].lit) + solver.numWatches(index[2].lit) +
                       solver.numWatches(index[3].lit) + solver.numWatches(index[4].lit);
                CPPUNIT_ASSERT(numW == 0);
            }
