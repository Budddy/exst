    ...
    namespace Test {
        ...
        public:
            ...
            void testClauseActivity() {
                ...
                solver->add(cl2);
                while (exp != 0) {
                    CPPUNIT_ASSERT(cl1->activity().activity() == cl2->activity().activity() &&
                                   cl1->activity().activity() == exp);
                    exp >>= 1;
            ...
            void testStrengthenToUnary() {
                ...
                clLits.push_back(b);
                clLits.push_back(~a);
                ClauseInfo extra(Constraint_t::learnt_conflict);
            ...
            void testStrengthenContracted() {
                ...
                CPPUNIT_ASSERT_EQUAL(uint32(sizeof(Clause) + (9 * sizeof(Literal))),
                                     ((Clause *) c)->computeAllocSize());
            }
            ...
            void testStrengthenBug() {
                ...
                ClauseHead *c = Clause::newContractedClause(*solver,
                                                            ClauseRep::create(&clause[0], (uint32) clause.size(),
                                                                              ClauseInfo(
                                                                                      Constraint_t::learnt_conflict)),
                                                            5, true);
                ...
                for (uint32 i = 0; i != clause.size(); ++i) {
                    CPPUNIT_ASSERT(std::find(clause2.begin(), clause2.end(), clause[i]) != clause2.end() ||
                                   clause[i] == posLit(4));
                }
            }
            ...
            void testStrengthenContractedNoExtend() {
                ...
                ClauseRep x = ClauseRep::create(&clause[0], (uint32) clause.size(),
                                                ClauseInfo(Constraint_t::learnt_conflict));
            ...
            void testStrengthenLockedEarly() {
                ...
                CPPUNIT_ASSERT(!remove || solver->reason(d).type() != Antecedent::generic_constraint ||
                               solver->reason(d).constraint() != clause);
            }
            ...
            void testNewContractedClause() {
                ...
                ClauseRep x = ClauseRep::create(&clLits[0], (uint32) clLits.size(),
                                                ClauseInfo(Constraint_t::learnt_conflict));
            ...
            void testLoopFormulaSatisfied() {
                ...
                TypeSet ts, other;
                ts.addSet(t);
                other.addSet(Constraint_t::learnt_conflict);
                LitVec free;
        ...
        private:
            ...
            Clause *createClause(LitVec &lits, const ClauseInfo &info) {
                uint32 flags = ClauseCreator::clause_explicit | ClauseCreator::clause_no_add |
                               ClauseCreator::clause_no_prepare;
