...
namespace Clasp {
    namespace Test {
        ...
        public:
            ...
            void testRunIncremental(Result::Base stop, int maxStep, int minStep, Result::Base expectedRes,
                                    int expectedSteps) {
                ClaspConfig config;
                ClaspFacade f;
                f.ctx.enableStats(2);
            ...
            void testIncrementalMin() {
                ...
                asp.setAtomName(1, "a").setAtomName(2, "b").setAtomName(3, "c");
                asp.startRule(Clasp::Asp::CHOICERULE).addHead(1).addHead(2).addHead(3).endRule();
                asp.startRule(Clasp::Asp::OPTIMIZERULE).addToBody(1, true).addToBody(2, true).addToBody(3,
                                                                                                        true).endRule();
                libclasp.prepare();
                CPPUNIT_ASSERT(libclasp.solve().sat());
            ...
            void testIncrementalLookaheadAddHeuristic() {
                ...
                libclasp.startAsp(config, true).setAtomName(1, "a").setAtomName(2, "b").startRule(
                        Asp::CHOICERULE).addHead(1).addHead(2).endRule();
                libclasp.prepare();
            ...
            void testIncrementalChangeLookahead() {
                ...
                libclasp.update(true);
                libclasp.prepare();
                Lookahead *look = static_cast<Lookahead *>(libclasp.ctx.master()->getPost(
                        PostPropagator::priority_reserved_look));
            ...
            void testIncrementalExtendLookahead() {
                ...
                static_cast<Asp::LogicProgram &>(libclasp.update(true)).startRule(Asp::CHOICERULE).addHead(3).addHead(
                        4).endRule();
                libclasp.prepare();
                Lookahead *look = static_cast<Lookahead *>(libclasp.ctx.master()->getPost(
                        PostPropagator::priority_reserved_look));
            ...
            void testIncrementalRemoveSolver() {
                ...
                libclasp.startAsp(config, true)
                        ...
                        .startRule(Asp::CONSTRAINTRULE, 3).addHead(7).addToBody(1, true).addToBody(2, true).addToBody(3,
                                                                                                                      true).addToBody(
                                4, true).endRule()
                        .setCompute(7, false);
                libclasp.prepare();
                ...
                static_cast<Asp::LogicProgram &>(libclasp.update(true)).startRule().addHead(7).addToBody(1,
                                                                                                         true).addToBody(
                        2, true).endRule();
                libclasp.prepare();
