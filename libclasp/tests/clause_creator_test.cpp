    ...
    namespace Test {
        ...
        public:
            ...
            void testCreateLearntClauseConflict() {
                ...
                solver().assume(a);
                solver().force(b, 0);
                solver().propagate();// level 1
                solver().assume(c);
                solver().propagate();                     // level 2
                solver().assume(d);
                solver().propagate();                     // level 3
                solver().assume(f);
                solver().propagate();                     // level 4
            ...
            void testCreateBogusUnit() {
                ...
                CPPUNIT_ASSERT(
                        ClauseCreator::status(solver(), &creator.lits()[0], &creator.lits()[0] + creator.size()) ==
                        ClauseCreator::status_sat);
            ...
            void testIntegrateSATBug4() {
                ...
                SharedLiterals *p(SharedLiterals::newShareable(cl, Constraint_t::learnt_other));
                CPPUNIT_ASSERT(
                        (ClauseCreator::status(solver(), p->begin(), p->end()) & ClauseCreator::status_unit) != 0);
            ...
            void testIntegrateNotConflictingBug() {
                ...
                CPPUNIT_ASSERT(
                        (ClauseCreator::status(solver(), p->begin(), p->end()) & ClauseCreator::status_unsat) != 0);
