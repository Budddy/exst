        ...
        public:
            ...
            void testAtomLookahead() {
                ...
                api.start(ctx, LogicProgram::AspOptions().noEq().noScc())
                        ...
                        .startRule().addHead(1).addToBody(2, true).addToBody(3, true).addToBody(4, true).addToBody(1,
                                                                                                                   false).endRule()
                        .startRule().addHead(2).addToBody(5, false).endRule()
            ...
            void testStrangeLookSeq() {
                SharedContext ctx;
                Lookahead::Params p(Lookahead::body_lookahead);
                p.limit(3);
            ...
            void testStrangeLookSeq2() {
                SharedContext ctx;
                Lookahead::Params p(Lookahead::atom_lookahead);
                p.limit(2);
                ctx.master()->setHeuristic(UnitHeuristic::restricted(new SelectFirst));
                Literal a = posLit(ctx.addVar(Var_t::atom_var));
                Literal b = posLit(ctx.addVar(Var_t::atom_var));
            ...
            void testDomLevel() {
                ...
                Solver &s = *ctx.master();
                LogicProgram api;
                api.start(ctx).setAtomName(1, "a").setAtomName(2, "b").startRule(Asp::CHOICERULE).addHead(1).addHead(
                        2).endRule();
                addDomRule(api, "_heuristic(a,sign,1)");
            ...
            void testDomPrefixBug() {
                ...
                Solver &s = *ctx.master();
                LogicProgram api;
                api.start(ctx).setAtomName(1, "a").setAtomName(2, "ab").setAtomName(3, "abc").startRule(
                        Asp::CHOICERULE).addHead(1).addHead(2).addHead(3).endRule();
