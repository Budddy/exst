...
//! Possible types of a variable.
    struct Var_t {
        enum Type {
            atom_var = 1, body_var = 2, atom_body_var = atom_var | body_var
        };
 ...
 */
    ...
    public:
        ...
        symbol_type &addUnique(key_type id, const char *name) {
            assert((lastSort_ == 0 || map_[lastSort_ - 1].first < id) &&
                   "Symbol table: Invalid id in incremental step!");
    ...
    private:
        ...
        bool unique() const {
            for (const_iterator it = map_.begin() + lastSort_, end = map_.end(), n = it + (it != end);
                 n != end; it = n++) {
