...
// see below
        ...
        public:
            ...
            const_left_iterator left_begin() const {
                return const_left_iterator(reinterpret_cast<const left_type *>(begin()));
            }
            const_left_iterator left_end() const {
                return const_left_iterator(reinterpret_cast<const left_type *>(buf_ + left_));
            }
            ...
            const_right_iterator right_begin() const {
                return const_right_iterator(reinterpret_cast<const right_type *>(end()));
            }
            const_right_iterator right_end() const {
                return const_right_iterator(reinterpret_cast<const right_type *>(buf_ + right_));
            }
            ...
            enum {
                block_size = ((sizeof(max_type) + (sizeof(align_type) - 1)) / sizeof(align_type)) * sizeof(align_type)
            };
        protected:
...
// on parameter i
        ...
        private:
            ...
            enum {
                padding = sizeof(with_extra_type) - (sizeof(no_extra_type) + sizeof(align_type))
            };
            enum {
                size_with_pad = sizeof(no_extra_type) + padding
            };
            enum {
                store_extra = (i > size_with_pad) && (i - size_with_pad) >= base_type::block_size
            };
            enum {
                inline_raw_cap = store_extra ? ((i - size_with_pad) / base_type::block_size) * base_type::block_size : 0
            };
        public:
 ...
 */
    ...
    void left_right_sequence<L, R, i>::copy(const left_right_sequence &other) {
        ...
        std::memcpy(this->right(), const_cast<left_right_sequence &>(other).right(),
                    other.right_size() * sizeof(right_type));
    }
