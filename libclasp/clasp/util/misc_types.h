...
//! Base class for library events.
    struct Event {
        enum Subsystem {
            subsystem_facade = 0, subsystem_load = 1, subsystem_prepare = 2, subsystem_solve = 3
        };
        enum Verbosity {
            verbosity_quiet = 0, verbosity_low = 1, verbosity_high = 2, verbosity_max = 3
        };
        explicit Event(Subsystem sys, uint32 evId, Verbosity verbosity) : system(sys), verb(verbosity), op(0),
                                                                          id(evId) { }
...
//! A log message.
    struct LogEvent : Event_t<LogEvent> {
        ...
        LogEvent(Subsystem sys, Verbosity verb, LogType t, const Solver *s, const char *what) : Event_t<LogEvent>(sys,
                                                                                                                  verb),
                                                                                                solver(s), msg(what) {
...
//! Creates a low priority message event.
    inline LogEvent message(Event::Subsystem sys, const char *what, const Solver *s = 0) {
        return LogEvent(sys, Event::verbosity_high, LogEvent::message, s, what);
    }
    template<Event::Verbosity V>
    inline LogEvent message(Event::Subsystem sys, const char *what, const Solver *s = 0) {
        return LogEvent(sys, V, LogEvent::message, s, what);
    }
//! Creates a high priority warning event.
    inline LogEvent warning(Event::Subsystem sys, const char *what, const Solver *s = 0) {
        return LogEvent(sys, Event::verbosity_quiet, LogEvent::warning, s, what);
    }
...
//! Event type for reporting models via generic event handler.
    ...
    const ToType *event_cast(const EvType &ev) { return ev.id == ToType::id_s ? static_cast<const ToType *>(&ev) : 0; }
    ...
    public:
        explicit EventHandler(Event::Verbosity verbosity = Event::verbosity_quiet) {
            for (uint32 i = 0; i != sizeof(verbosity_) /
                                    sizeof(verbosity_[0]); ++i) { verbosity_[i] = static_cast<uint8>(verbosity); }
        }
...
//! An unary operator function that calls p->release()
    ...
    struct IsNull {
        template<class T>
        bool operator()(const T &p) const { return p == 0; }
    };
 ...
 */
    ...
    public:
        ...
        T *release() {
            store_clear_bit(ptr_, 0);
            return get();
        }
        T *acquire() {
            store_set_bit(ptr_, 0);
            return get();
        }
        void reset(T *x) {
            if (x != get() && is_owner()) {
