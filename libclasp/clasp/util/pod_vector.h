...
namespace bk_lib {
    namespace detail {
        ...
        struct IterType {
            ...
            static T &makeT();
            enum {
                ptr = sizeof(isPtr(makeT())) == sizeof(yes_type)
            };
            enum {
                num = sizeof(isLong(makeT())) == sizeof(yes_type)
            };
            enum {
                value = ptr ? 1 : num ? 2 : 0
            };
 ...
 */
    ...
    public:
         ...
         */
        template<class Iter>
        pod_vector(Iter first, Iter last, const allocator_type &a = allocator_type(),
                   typename detail::disable_if<detail::IterType<Iter>::num>::type * = 0)
                : ebo_(0, a) {
    ...
    private:
        ...
        void insert_range(iterator pos, It first, It last, std::random_access_iterator_tag,
                          typename detail::disable_if<detail::same_type<pointer, It>::value == 0 &&
                                                      detail::same_type<const_pointer, It>::value == 0>::type * = 0) {
