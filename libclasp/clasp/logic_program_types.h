        ...
        inline RuleType &operator++(RuleType &x) {
            switch (x) {
                ...
                case CHOICERULE:
                    return x = WEIGHTRULE;
                case WEIGHTRULE:
                    return x = OPTIMIZERULE;
                case OPTIMIZERULE:
                    return x = DISJUNCTIVERULE;
                case DISJUNCTIVERULE:
                    return x = ENDRULE;
                default:
                    return x = BASICRULE;
 ...
 */
        struct PrgEdge {
            //! Type of edge.
            enum EdgeType {
                NORMAL_EDGE = 0, GAMMA_EDGE = 1, CHOICE_EDGE = 2, GAMMA_CHOICE_EDGE = 3
            };
            //! Type of adjacent node.
            enum NodeType {
                BODY_NODE = 0, ATOM_NODE = 1, DISJ_NODE = 2
            };
 ...
 */
        ...
        struct BodyInfo {
            enum BodyType {
                NORMAL_BODY = 0, COUNT_BODY = 1, SUM_BODY = 2
            };
            void reset() {
                lits.clear();
                hash = 0;
            ...
            RuleType ruleType() const {
                const RuleType rt[3] = {BASICRULE, CONSTRAINTRULE, WEIGHTRULE};
                return rt[type()];
            }
 ...
 */
        ...
        public:
            enum Simplify {
                no_simplify = 0, force_simplify = 1
            };
            ...
            Literal assumption() const {
                return freeze_ > uint32(freeze_free) ? literal() ^ (freeze_ == freeze_false) : posLit(0);
            }
...
//! A (rule) body in a logic program.
        ...
        public:
            ...
            weight_t bound() const {
                return static_cast<weight_t>(type() == BodyInfo::NORMAL_BODY ? (weight_t) size() : boundImpl());
            }
            //! Returns the sum of the subgoals weights, or size() if the body is not a SUM_BODY.
            weight_t sumW() const {
                return static_cast<weight_t>(type() != BodyInfo::SUM_BODY ? (weight_t) size() : data_.ext[0]->sumW);
            }
            //! Returns the idx'th subgoal as a literal.
            Literal goal(uint32 idx) const {
                assert(idx < size());
                return *(goals_begin() + idx);
            }
            //! Returns the weight of the idx'th subgoal.
            weight_t weight(uint32 idx) const {
                assert(idx < size());
                return type() != BodyInfo::SUM_BODY ? 1 : data_.ext[0]->weights[idx];
            }
        ...
        private:
            ...
            weight_t boundImpl() const {
                return static_cast<weight_t>(!hasWeights() ? (weight_t) data_.lits[0] : data_.ext[0]->bound);
            }
            ...
            Literal *goals_begin() {
                return reinterpret_cast<Literal *>( data_.lits + (type() == BodyInfo::NORMAL_BODY ? 0
                                                                                                  : SumExtra::LIT_OFFSET));
            }

            struct SumExtra {
                enum {
                    LIT_OFFSET = sizeof(void *) / sizeof(uint32)
                };
...
//! The head of a disjunctive rule.
        ...
        inline ValueRep getMergeValue(const PrgNode *lhs, const PrgNode *rhs) {
            return static_cast<ValueRep>(
                    std::min(static_cast<ValueRep>(lhs->value() - 1), static_cast<ValueRep>(rhs->value() - 1)) + 1);
        }
        ...
        private:
            ...
            bool doVisit(PrgNode *n, bool seen = true) const {
                return !n->ignoreScc() && n->relevant() && n->hasVar() && (!seen || !n->seen());
            }
