...
//! Type for storing a model.
    struct Model {
        enum Type {
            model_sat = 0, model_cons = 1, max_value = 1
        };
        ...
        uint32 sId :16;
        // id of solver that found the model
        uint32 type:14;
        // type of model
        uint32 opt : 1;
        // whether the model is optimal w.r.t costs (0: unknown)
 ...
 */
    ...
    private:
        enum Flag {
            flag_path_disjoint = 4u, flag_model_heuristic = 8u
        };
        enum {
            clear_state_mask = ~uint32(value_true | value_false)
        };
