...
//! Type storing a constraint's activity.
    struct Activity {
        enum {
            LBD_SHIFT = 7, MAX_LBD = (1 << LBD_SHIFT) - 1, MAX_ACT = (1 << (32 - LBD_SHIFT)) - 1
        };
 ...
 */
    ...
    public:
        enum Type {
            generic_constraint = 0, ternary_constraint = 1, binary_constraint = 2
        };
        ...
        bool learnt() const {
            return data_ && (data_ & 3u) == 0 && constraint()->type() != Constraint_t::static_constraint;
        }
