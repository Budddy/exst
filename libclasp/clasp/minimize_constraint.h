 ...
 */
    ...
    public:
        ...
        bool setMode(MinimizeMode m, const SumVec &bound) {
            return setMode(m, bound.empty() ? 0 : &bound[0], (uint32) bound.size());
        }
        void resetBounds();
        ...
        void add(wsum_t *lhs, const WeightLiteral &lit) const {
            if (weights.empty()) *lhs += lit.second; else
                add(lhs, &weights[lit.second]);
        }
        ...
        void sub(wsum_t *lhs, const WeightLiteral &lit, uint32 &aLev) const {
            if (weights.empty()) *lhs -= lit.second; else
                sub(lhs, &weights[lit.second], aLev);
        }
...
//! Helper class for creating minimize constraints.
    ...
    private:
        ...
        weight_t addLitImpl(uint32 lev, WeightLiteral lit) {
            if (lit.second > 0) {
                lits_.push_back(LitRep(lit.first, new Weight(lev, lit.second)));
                return 0;
            }
            if (lit.second < 0) {
                lits_.push_back(LitRep(~lit.first, new Weight(lev, -lit.second)));
 ...
 */
    ...
    private:
        enum PropMode {
            propagate_new_sum, propagate_new_opt
        };
        union UndoInfo;
...
//! Minimization via unsat cores.
    ...
    private:
        ...
        // active level
        uint32 valid_ :  1;
        // valid w.r.t active generation?
        uint32 sat_   :  1;
        // update because of model
        uint32 pre_   :  1;
