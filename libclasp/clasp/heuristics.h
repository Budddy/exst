...
//! Computes a moms-like score for var v.
    ...
    struct HeuParams {
        ...
        HeuParams &other(uint32 x) {
            otherScore = static_cast<uint8>(x);
            return *this;
        }
        HeuParams &init(uint32 moms) {
            initScore = static_cast<uint8>(moms);
            return *this;
        }
        HeuParams &score(uint32 x) {
            resScore = static_cast<uint8>(x);
            return *this;
        }
        uint8 initScore; // currently {no, moms}
        uint8 otherScore;
        // currently {no, loop, all, heu}
 ...
 */
    ...
    private:
        ...
        struct Order {
            ...
            void inc(Literal p, uint16 f) {
                if (f) {
                    score[p.var()].decay(decay, huang);
                    score[p.var()].act += f;
                }
            }
...
//! Domain modification for one literal.
    struct DomEntry {
        ...
        enum Modifier {
            mod_factor = 0, mod_level = 1, mod_sign = 2, mod_init = 3, mod_tf = 4
        };
        ...
        // one of Modifier
        uint32 pref: 2;
        // pref value (in case of mod_sign or mod_tf)
 ...
 */
    class DomainHeuristic : public ClaspVsids_t<DomScore>, private Constraint {
    public:
        typedef ClaspVsids_t<DomScore> BaseType;
        enum GlobalPreference {
            gpref_atom = 0, gpref_scc = 1, gpref_hcc = 2, gpref_disj = 4, gpref_min = 8, gpref_show = 16
        };
        enum GlobalModifier {
            gmod_none = 0, gmod_level = 1, gmod_spos = 2, gmod_true = 3, gmod_sneg = 4, gmod_false = 5
        };
