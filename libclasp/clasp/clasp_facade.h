...
//! Configuration object for configuring solving via the ClaspFacade.
    ...
    public:
        ...
        SolveOptions solve;
        /*!< Options for solve algorithm and enumerator. */
...
//! A (positive) numeric value.
    struct ExpectedQuantity {
        enum Error {
            error_none = 0, error_unknown_quantity = 1, error_ambiguous_quantity = 2, error_not_available = 3
        };
...
//! Provides a simplified interface to the services of the clasp library.
    ...
    public:
        ...
        struct StepStart : Event_t<StepStart> {
            explicit StepStart(const ClaspFacade &f) : Event_t<StepStart>(subsystem_facade, verbosity_quiet),
                                                       facade(&f) { }
            const ClaspFacade *facade;
        };
        ...
        struct StepReady : Event_t<StepReady> {
            explicit StepReady(const Summary &x) : Event_t<StepReady>(subsystem_facade, verbosity_quiet),
                                                   summary(&x) { }
