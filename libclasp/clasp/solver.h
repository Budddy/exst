...
//! Parameter-Object for managing search limits.
    struct SearchLimits {
        explicit SearchLimits(uint64 conf = UINT64_MAX)
                : conflicts(conf), local(UINT64_MAX), memLimit(UINT64_MAX), dynamic(0), learnts(UINT32_MAX) {
        }
 ...
 */
    ...
    public:
        ...
        bool allowImplicit(const ClauseRep &c) const {
            return c.isImp()
                   ? shared_->allowImplicit(c.info.type()) && !c.info.aux() && (c.prep == 1 ||
                                                                                (!auxVar(c.lits[0].var()) &&
                                                                                 !auxVar(c.lits[1].var()) &&
                                                                                 (c.size == 2 ||
                                                                                  !auxVar(c.lits[2].var()))))
        ...
        bool propagateUntil(PostPropagator *p) {
            assert((!p || post_.active()) && "OP not allowed during init!");
            return unitPropagate() && (p == post_.active() || post_.propagate(*this, p));
        }
        ...
        enum UndoMode {
            undo_default = 0u, undo_pop_bt_level = 1u, undo_save_phases = 2u
        };
        ...
        Literal trueLit(Var v) const {
            ...
            return Literal(v, valSign(value(v)));
        }
        ...
        bool learntLimit(const SearchLimits &x) const {
            return numLearntConstraints() > x.learnts || memUse_ > x.memLimit;
        }
        ...
        void markSeen(Var v) {
            assert(validVar(v));
            assign_.setSeen(v, 3u);
        }
        void markSeen(Literal p) {
            assert(validVar(p.var()));
            assign_.setSeen(p.var(), uint8(1 + p.sign()));
        }
    ...
    private:
        struct DLevel {
            explicit DLevel(uint32 pos = 0, ConstraintDB *u = 0)
                    : trailPos(pos), marked(0), freeze(0), undo(u) { }
            uint32 trailPos : 30;
            uint32 marked   :  1;
            uint32 freeze   :  1;
            ConstraintDB *undo;
        ...
        uint32 lastSimp_ :30;
        // number of top-level assignments on last call to simplify
        uint32 shufSimp_ : 1;
        // shuffle db on next simplify?
    ...
    inline Literal Solver::defaultLit(Var v) const {
        switch (strategy_.signDef) {
            default: //
            case SolverStrategies::sign_atom:
                return Literal(v, !varInfo(v).has(VarInfo::BODY));
            case SolverStrategies::sign_pos :
                return posLit(v);
            case SolverStrategies::sign_neg :
                return negLit(v);
            case SolverStrategies::sign_rnd :
                return Literal(v, rng.drand() < 0.5);
            case SolverStrategies::sign_disj:
                return Literal(v, !varInfo(v).has(VarInfo::BODY | VarInfo::DISJ));
        }
    }
//@}
 ...
 */
    ...
    public:
         ...
         */
        virtual void newConstraint(const Solver &, const Literal * /* first */, LitVec::size_type /* size */,
                                   ConstraintType /* t */) { }
