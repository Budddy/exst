...
#undef MAX_MEM
    ...
    struct SumQueue {
        ...
        void resetQueue() {
            sumLbd = sumCfl = samples = 0;
            queue.clear();
        }
        void resetGlobal() {
            globalSumLbd = globalSumCfl = globalSamples = 0;
            resetQueue();
        }
        ...
        bool isRestart() const {
            return full() && (lbd ? (avgLbd() * lim) > globalAvgLbd() : (avgCfl() * lim) > globalAvgCfl());
        }
    ...
    private:
        SumQueue(uint32 size)
                : globalSumLbd(0), globalSumCfl(0), globalSamples(0), sumLbd(0), sumCfl(0), samples(0), upForce(16000),
                  upCfl(0), nRestart(0), lim(0.0f), lbd(true), queue(size) {
        }
...
//! A struct for aggregating statistics maintained in a solver object.
    ...
    inline void SolverStats::addDistributed(uint32 lbd, ConstraintType) {
        if (extra) {
            ++extra->distributed;
    ...
    inline void SolverStats::addPath(const LitVec::size_type &sz) {
        if (extra) {
            ++extra->gps;
            extra->gpLits += sz;
        }
    ...
    inline void SolverStats::addTest(bool partial) {
        if (extra) {
            ++extra->hccTests;
            extra->hccPartial += (uint32) partial;
        }
    }
    inline void SolverStats::addModel(uint32 DL) {
        if (extra) {
            ++extra->models;
            extra->modelLits += DL;
        }
...
//! Type storing initial information on a (learnt) clause.
    ...
    public:
        ...
        enum {
            MAX_LBD = Activity::MAX_LBD, MAX_ACTIVITY = (1 << 21) - 1
        };
        ...
        self_type &setType(ConstraintType t) {
            type_ = static_cast<uint32>(t);
            return *this;
        }
        self_type &setActivity(uint32 act) {
            act_ = std::min(act, (uint32) MAX_ACTIVITY);
            return *this;
        }
        self_type &setTagged(bool b) {
            tag_ = static_cast<uint32>(b);
            return *this;
        }
        self_type &setLbd(uint32 a_lbd) {
            lbd_ = std::min(a_lbd, (uint32) MAX_LBD);
            return *this;
        }
        self_type &setAux(bool b) {
            aux_ = static_cast<uint32>(b);
            return *this;
        }
    private:
...
//! Primitive representation of a clause.
    struct ClauseRep {
        static ClauseRep create(Literal *cl, uint32 sz, const ClauseInfo &i = ClauseInfo()) {
            return ClauseRep(cl, sz, false, i);
        }
        static ClauseRep prepared(Literal *cl, uint32 sz, const ClauseInfo &i = ClauseInfo()) {
            return ClauseRep(cl, sz, true, i);
        }
        ClauseRep(Literal *cl = 0, uint32 sz = 0, bool p = false, const ClauseInfo &i = ClauseInfo()) : info(i),
                                                                                                        size(sz),
                                                                                                        prep(uint32(p)),
                                                                                                        lits(cl) { }
        ClauseInfo info;
 ...
 */
    ...
    public:
        enum {
            HEAD_LITS = 3, MAX_SHORT_LEN = 5, MAX_LBD = (1 << 5) - 1, TAGGED_CLAUSE = 1023, MAX_ACTIVITY = (1 << 15) - 1
        };
        ...
        void resetActivity(Activity a) {
            info_.data.act = std::min(a.activity(), uint32(MAX_ACTIVITY));
            info_.data.lbd = std::min(a.lbd(), uint32(MAX_LBD));
        }
    ...
    struct ReasonStore32 : PodVector<Antecedent>::type {
        ...
        struct value_type {
            value_type(const Antecedent &a, uint32 d) : ante_(a) {
                if (d != UINT32_MAX) {
                    encode(ante_, d);
                    assert(data() == d && ante_.type() == Antecedent::generic_constraint);
                }
            }
 ...
 */
    struct ValueSet {
        ValueSet() : rep(0) { }
        enum Value {
            user_value = 0x03u, saved_value = 0x0Cu, pref_value = 0x30u, def_value = 0xC0u
        };
 ...
 */
    ...
    public:
        ...
        typedef bk_lib::detail::if_then_else<
                sizeof(Constraint *) == sizeof(uint64), ReasonStore64, ReasonStore32>::type ReasonVec;
        ...
        uint32 data(Var v) const {
            assert(v < reason_.dataSize());
            return reason_.data(v);
        }
        ...
        void undoTrail(LitVec::size_type first, bool save) {
            if (!save) { popUntil<&Assignment::clear>(trail[first]); }
                ...
                requestPrefs();
                popUntil<&Assignment::saveAndClear>(trail[first]);
            }
            qReset();
        }
        ...
        bool markUnits() {
            while (units_ != front) { setSeen(trail[units_++].var(), 3u); }
            return true;
        }
...
//! Stores information about a literal that is implied on an earlier level than the current decision level.
    struct ImpliedLiteral {
        ...
        ImpliedLiteral(Literal a_lit, uint32 a_level, const Antecedent &a_ante, uint32 a_data = UINT32_MAX)
                : lit(a_lit), level(a_level), ante(a_ante, a_data) {
        }
        Literal lit;
        /**< The implied literal */
        uint32 level;
...
//! A type for storing ImpliedLiteral objects.
    ...
    struct CCMinRecursive {
        enum State {
            state_open = 0, state_poison = 1, state_removable = 2
        };
