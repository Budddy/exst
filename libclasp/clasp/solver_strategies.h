 ...
 */
    ...
    public:
        enum Type {
            geometric_schedule = 0, arithmetic_schedule = 1, luby_schedule = 2, user_schedule = 3
        };
        ...
        static ScheduleStrategy luby(uint32 unit, uint32 limit = 0) {
            return ScheduleStrategy(luby_schedule, unit, 0, limit);
        }
        ...
        static ScheduleStrategy geom(uint32 base, double grow, uint32 limit = 0) {
            return ScheduleStrategy(geometric_schedule, base, grow, limit);
        }
        ...
        static ScheduleStrategy arith(uint32 base, double add, uint32 limit = 0) {
            return ScheduleStrategy(arithmetic_schedule, base, add, limit);
        }
    ...
    struct SolverStrategies {
        ...
        enum WatchInit {
            watch_first = 0, watch_rand = 1, watch_least = 2
        };
        enum UpdateMode {
            update_on_propagate = 0, update_on_conflict = 1
        };
...
//! Parameter-Object for configuring a solver.
    struct SolverParams : SolverStrategies {
        ...
        // 32-bit
        uint32 optStrat  : 1;
        /*!< Optimization strategy (see MinimizeMode_t::Strategy).*/
        uint32 optParam  : 3;
        /*!< Parameter for optimization strategy (see MinimizeMode_t::BBOption / MinimizeMode_t::UscOption). */
        uint32 optHeu    : 2;
        /*!< Set of optimize heuristics. */
        uint32 heuId     : 3;
        /*!< Type of decision heuristic.   */
        uint32 heuScore  : 2;
        /*!< Type of scoring during resolution. */
        uint32 heuOther  : 2;
        ...
        uint32 forgetSet : 4;
        /*!< What to forget on (incremental step). */
        uint32 domPref   : 5;
 ...
 */
    struct RestartParams {
        RestartParams()
                : sched(), counterRestart(0), counterBump(9973), shuffle(0), shuffleNext(0), upRestart(0), cntLocal(0),
                  dynRestart(0) { }
        enum SeqUpdate {
            seq_continue = 0, seq_repeat = 1, seq_disable = 2
        };
 ...
 */
    struct ReduceParams {
        ReduceParams() : cflSched(ScheduleStrategy::none()), growSched(ScheduleStrategy::def()), fInit(1.0f / 3.0f),
                         fMax(3.0f), fGrow(1.1f), initRange(10, UINT32_MAX), maxRange(UINT32_MAX), memMax(0) { }
        void disable();
        uint32 prepare(bool withLookback);
 ...
 */
    struct SolveParams {
        ...
        struct FwdCheck {
            /*!< Options for partial checks in DLP-solving; */
            uint32 initHigh : 24;
            /*!< Init high level to this DL (0 = max level) */
            uint32 highPct  :  7;
...
//! Parameters for a SharedContext object.
    struct ContextParams {
        ...
        ContextParams() : shareMode(share_auto), stats(0), shortMode(short_implicit), seed(1), hasConfig(0),
                          cliConfig(0), cliId(0), cliMode(0) { }
        SatPreParams satPre;
...
//! Simple factory for decision heuristics.
    struct Heuristic_t {
        enum Type {
            heu_default = 0, heu_berkmin = 1, heu_vsids = 2, heu_vmtf = 3, heu_domain = 4, heu_unit = 5, heu_none = 6
        };
