...
//! Program statistics for *one* incremental step.
        ...
        public:
            ...
            void upRule(RuleType t, int32 i) {
                RPair &x = rules_[ruleIndex(t)];
                x.first += i;
                x.second += i;
            }
            void trRule(RuleType t, uint32 i) {
                --rules_[ruleIndex(t)].second;
                rules_[0].first -= i;
            }
 ...
 */
        ...
        public:
            ...
            struct AspOptions {
                ...
                AspOptions() : erMode(mode_native), iters(5), noSCC(0), dfOrder(0), backprop(0), normalize(0),
                               suppMod(0), noGamma(0), freezeShown(0) { }
            ...
            PrgHead *getHead(PrgEdge it) const {
                return it.isAtom() ? (PrgHead *) getAtom(it.node()) : (PrgHead *) getDisj(it.node());
            }
            PrgNode *getSupp(PrgEdge it) const {
                return it.isBody() ? (PrgNode *) getBody(it.node()) : (PrgNode *) getDisj(it.node());
            }
            ...
            HeadIter disj_begin() const {
                return disjunctions_.empty() ? 0 : reinterpret_cast<HeadIter>(&disjunctions_[0]);
            }
        ...
        private:
            ...
            struct Incremental {
                Incremental();
                uint32 startAtom;
                // first atom of current iteration
