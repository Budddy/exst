 ...
 */
    struct ProblemStats {
        ...
        void diff(const ProblemStats &o) {
            ...
            vars_eliminated =
                    std::max(vars_eliminated, o.vars_eliminated) - std::min(vars_eliminated, o.vars_eliminated);
            ...
            constraints_binary = std::max(constraints_binary, o.constraints_binary) -
                                 std::min(constraints_binary, o.constraints_binary);
            constraints_ternary = std::max(constraints_ternary, o.constraints_ternary) -
                                  std::min(constraints_ternary, o.constraints_ternary);
        }
...
//! Stores static information about a variable.
    struct VarInfo {
        ...
        VarType type() const {
            return has(VarInfo::EQ) ? Var_t::atom_body_var : VarType(Var_t::atom_var + has(VarInfo::BODY));
        }
 ...
 */
    ...
    public:
        ...
        enum InitMode {
            init_share_symbols, init_copy_symbols
        };
        enum ResizeMode {
            mode_reserve = 0u, mode_add = 1u, mode_remove = 2u, mode_resize = 3u
        };
        /*!
         * \name Configuration
         */
       ...
       */
        ...
        void setPreserveModels(bool b = true) {
            share_.satPreM = b ? SatPreParams::prepro_preserve_models : SatPreParams::prepro_preserve_sat;
        }
        ...
        bool preserveModels() const {
            return static_cast<SatPreParams::Mode>(share_.satPreM) == SatPreParams::prepro_preserve_models;
        }
        //! Returns whether physical sharing is enabled for constraints of type t.
        bool physicalShare(ConstraintType t) const {
            return (share_.shareM & (1 + (t != Constraint_t::static_constraint))) != 0;
        }
        ...
        bool allowImplicit(ConstraintType t) const {
            return t != Constraint_t::static_constraint ? share_.shortM != ContextParams::short_explicit : !isShared();
        }
        //@}
        ...
        void resizeVars(uint32 maxVar) {
            varInfo_.resize(std::max(Var(1), maxVar));
            problem_.vars = numVars();
        }
        ...
        void setProject(Var v, bool b) {
            if (b != varInfo(v).has(VarInfo::PROJECT))
        ...
        void mark(Literal p) {
            ...
            varInfo_[p.var()].rep |= (VarInfo::MARK_P + p.sign());
        }
        void unmark(Var v) {
            assert(validVar(v));
            varInfo_[v].rep &= ~(VarInfo::MARK_P | VarInfo::MARK_N);
        }
    ...
    private:
        ...
        // size of master's top-level after last init
        struct Share {               // Additional data
            ...
            Share() : count(1), winner(0), shareM((uint32) ContextParams::share_auto), shortM(0), frozen(0), seed(0),
                      satPreM(0) { }
        } share_;
