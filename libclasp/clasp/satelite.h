 ...
 */
        ...
        public:
            ...
            struct Progress : public Event_t<Progress> {
                enum EventOp {
                    event_algorithm = '*', event_bce = 'B', event_var_elim = 'E', event_subsumption = 'S',
                };
                Progress(SatElite *p, EventOp o, uint32 i, uint32 m) : Event_t<Progress>(Event::subsystem_prepare,
                                                                                         Event::verbosity_high),
                                                                       self(p), cur(i), max(m) {
                    op = (uint32) o;
                }
                SatElite *self;
        ...
        private:
            ...
            struct OccurList {
                ...
                ClRange clauseRange() const {
                    return ClRange(const_cast<ClWList &>(refs).left_begin(), const_cast<ClWList &>(refs).left_end());
                }
                void clear() {
                    this->~OccurList();
                    new(this) OccurList();
                ...
                void removeWatch(uint32 clId) {
                    refs.erase_right(std::find(refs.right_begin(), refs.right_end(), clId));
                }
