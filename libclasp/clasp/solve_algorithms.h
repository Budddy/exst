...
//! Basic (sequential) solving using given solving options.
    ...
    struct BasicSolveEvent : SolveEvent<BasicSolveEvent> {
        enum EventOp {
            event_none = 0, event_deletion = 'D', event_exit = 'E', event_grow = 'G', event_restart = 'R'
        };
        BasicSolveEvent(const Solver &s, EventOp a_op, uint64 cLim, uint32 lLim) : SolveEvent<BasicSolveEvent>(s,
                                                                                                               verbosity_max),
                                                                                   cLimit(cLim), lLimit(lLim) {
            op = a_op;
        }
