    ...
    private:
        ...
        struct AddSource { // an atom in a body has a new source, check if body is now a valid source
            ...
            void operator()(NodeId bId) const {
                ...
                if (--self->bodies_[bId].lower_or_ext == 0 &&
                    !self->solver_->isFalse(n.node->lit)) { self->forwardSource(n); }
            }
            // extended body
        ...
        struct RemoveSource {
            // an atom in a body has lost its source, check if body is no longer a valid source
        ...
        void resetTodo() {
            while (!todo_.empty()) { atoms_[todo_.pop_ret()].todo = 0; }
            todo_.clear();
        }
        void resetUfs() {
            while (!ufs_.empty()) { atoms_[ufs_.pop_ret()].ufs = 0; }
            ufs_.clear();
        }
        // -------------------------------------------------------------------------------------------
        ...
        // activeClause_[0] is the current unfounded atom
