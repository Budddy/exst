...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        void addVars(Solver &generator, const SccGraph &dep, const VarVec &atoms, const VarVec &bodies,
                     SharedContext &out);
...
//  au: for each atom a, au is true iff a is unfounded w.r.t to assignment of P.
    void SharedDependencyGraph::NonHcfComponent::ComponentMap::addVars(Solver &generator, const SccGraph &dep,
                                                                       const VarVec &atoms, const VarVec &bodies,
                                                                       SharedContext &comp) {
...
// ax(ai), au(ai) is the aux resp. output atom of ai in C.
    void SharedDependencyGraph::NonHcfComponent::ComponentMap::addBodyConstraints(const Solver &generator,
                                                                                  const SccGraph &dep, uint32 scc,
                                                                                  SharedContext &comp) {
...
// Maps the generator assignment given in s to a list of tester assumptions.
    void SharedDependencyGraph::NonHcfComponent::ComponentMap::mapGeneratorAssignment(const Solver &s,
                                                                                      const SccGraph &dep,
                                                                                      LitVec &assume) const {
        Literal gen;
        assume.clear();
...
// Maps the tester model given in s back to a list of unfounded atoms in the generator.
    ...
    bool SharedDependencyGraph::NonHcfComponent::ComponentMap::simplify(const Solver &generator, Solver &tester) {
        ...
        for (MapIt_c it = mapping.begin(), aEnd = it + numAtoms, end = mapping.end(); it != end; ++it) {
            ...
            if (atom) {
                if (!isFalse) {
                    ok = tester.force(m.atPos());
                    if (ok && rem) { *j++ = m; }
                }
                else {
                    ok = tester.force(~m.atPos()) && tester.force(~m.atUnf());
                    numAtoms -= (ok && rem);
                }
            }
            if (!ok) {
...
/////////////////////////////////////////////////////////////////////////////////////////
    SolveTestEvent::SolveTestEvent(const Solver &s, uint32 a_scc, bool part)
            : SolveEvent<SolveTestEvent>(s, Event::verbosity_max), result(-1), scc(a_scc), partial(part) {
    ...
    SharedDependencyGraph::NonHcfComponent::NonHcfComponent(const SharedDependencyGraph &dep, SharedContext &genCtx,
                                                            uint32 scc, const VarVec &atoms, const VarVec &bodies)
            : prg_(new SharedContext()), comp_(new ComponentMap()) {
    ...
    bool SharedDependencyGraph::NonHcfComponent::test(uint32 scc, const Solver &generator, const LitVec &assume,
                                                      VarVec &unfoundedOut) const {
