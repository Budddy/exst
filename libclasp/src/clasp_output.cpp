...
/////////////////////////////////////////////////////////////////////////////////////////
        ...
        void format(const Clasp::BasicSolveEvent &ev, char *out, uint32 outSize) {
            ...
            n = sprintf(buf, "%2u:%c|%7u/%-7u|%8u/%-8u|%10" PRIu64"/%-6.3f|%8" PRId64"/%-10" PRId64"|", s.id(),
                        static_cast<char>(ev.op), s.numFreeVars(),
                        (s.decisionLevel() > 0 ? s.levelStart(1) : s.numAssignedVars()), s.numConstraints(),
                        s.numLearntConstraints(), s.stats.conflicts,
        ...
        void format(const Clasp::SolveTestEvent &ev, char *out, uint32 outSize) {
            ...
            if (ev.result == -1) { n = sprintf(buf, "%2u:%c| HC: %-5u %-60s|", ev.solver->id(), ct, ev.scc, "..."); }
                ...
                n = sprintf(buf, "%2u:%c| HC: %-5u %-4s|%8u/%-8u|%10" PRIu64"/%-6.3f| T: %-15.3f|", ev.solver->id(), ct,
                            ev.scc, (ev.result == 1 ? "OK" : "FAIL"), ev.solver->numConstraints(),
                            ev.solver->numLearntConstraints(), ev.conflicts(),
                            ev.conflicts() / std::max(1.0, double(ev.choices())), ev.time
                );
            }
            null_term_copy(buf, n, out, outSize);
        }
...
/////////////////////////////////////////////////////////////////////////////////////////
        ...
        bool Output::onModel(const Solver &s, const Model &m) {
            ...
            if (modelQ() == print_best || (optQ() == print_best && m.costs)) {
                if (m.opt == 1 && !m.consequences()) {
                    printModel(s.symbolTable(), m, print_best);
                    clearModel();
                }
                else { saveModel(m); }
            }
...
/////////////////////////////////////////////////////////////////////////////////////////
        ...
        void JsonOutput::run(const char *solver, const char *version, const std::string *iBeg,
        ...
        void JsonOutput::printSummary(const ClaspFacade::Summary &run, bool final) {
            ...
            if (verbosity()) {
                if (run.result.interrupted()) {
                    printKeyValue(run.result.signal != SIGALRM ? "INTERRUPTED" : "TIME LIMIT", uint32(1));
                }
                pushObject("Models");
...
#define printKey(k)                  printf("%s%-*s: ", format[cat_comment], width_, (k))
        ...
        TextOutput::TextOutput(uint32 verbosity, Format f, const char *catAtom, char ifs) : Output(verbosity),
        ...
        void TextOutput::printSummary(const ClaspFacade::Summary &run, bool final) {
            ...
            if (verbosity() || run.stats()) {
                printBR(cat_comment);
                if (run.result.interrupted()) {
                    printKeyValue((run.result.signal != SIGALRM ? "INTERRUPTED" : "TIME LIMIT"), "%u\n", uint32(1));
                }
                printKey("Models");
                char buf[64];
            ...
            if (ev.verb <= verbosity()) {
                if (ev.system == 0) { setState(0, 0, 0); }
                    ...
                    if (ev.system == Event::subsystem_solve) { printSolveProgress(ev); }
                        ...
                        if (sat->op != SatElite::SatElite::Progress::event_algorithm) {
                            comment(2, "Sat-Prepro   : %c: %8u/%-8u\r", (char) sat->op, sat->cur, sat->max);
                        }
                        else if (sat->cur != sat->max) {
                            setState(0, 0, 0);
                            comment(2, "Sat-Prepro   :\r");
        ...
        void TextOutput::setState(uint32 state, uint32 verb, const char *m) {
            if (state != state_ && verb <= verbosity()) {
                double tEnd = RealTime::getTime();
                if (state_ == Event::subsystem_solve) {
                    comment(2, "%s\n", rowSep);
                    line_ = 20;
                }
                ...
                else if (state_ == Event::subsystem_solve) {
                    comment(1, "Solving...\n");
                    line_ = 0;
                }
            }
        ...
        void TextOutput::printSolveProgress(const Event &ev) {
            ...
            if (const BasicSolveEvent *be = event_cast<BasicSolveEvent>(ev)) { Clasp::Cli::format(*be, line, 128); }
            else if (const SolveTestEvent *te = event_cast<SolveTestEvent>(ev)) {
                Clasp::Cli::format(*te, line, 128);
                lEnd = te->result == -1 ? '\r' : '\n';
            }
#if WITH_THREADS
                ...
                printLN(cat_comment, "%2u:L| %-69s |", log->solver->id(), log->msg);
                return;
            }
            else { return; }
            ...
            if ((lEnd == '\n' && --line_ == 0) || newEvent) {
                if (line_ <= 0) {
                    ...
                    printf("%s%s\n"
                                   ...
                                   "%s%s\n", format[cat_comment], rowSep, format[cat_comment], format[cat_comment],
                           format[cat_comment], rowSep);
                }
        ...
        void TextOutput::visitLogicProgramStats(const Asp::LpStats &lp) {
            ...
            if (lp.eqs() > 0) {
                ...
                printf(" (Atom=Atom: %u Body=Body: %u Other: %u)\n", lp.eqs(Var_t::atom_var), lp.eqs(Var_t::body_var),
                       lp.eqs(Var_t::atom_body_var));
            }
            printKey("Tight");
            if (lp.sccs == 0) { printf("Yes"); }
            else if (lp.sccs != PrgNode::noScc) {
                printf("%-8s (SCCs: %u Non-Hcfs: %u Nodes: %u Gammas: %u)", "No", lp.sccs, lp.nonHcfs, lp.ufsNodes,
                       lp.gammas);
            }
            else { printf("N/A"); }
            printf("\n");
        ...
        void TextOutput::visitJumpStats(const JumpStats &st, bool) {
            printKeyValue("Backjumps", "%-8"
                    PRIu64, st.jumps);
            printf(" (Average: %5.2f Max: %3u Sum: %6" PRIu64")\n", st.avgJump(), st.maxJump, st.jumpSum);
            printKeyValue("  Executed", "%-8"
                    PRIu64, st.jumps - st.bounded);
            printf(" (Average: %5.2f Max: %3u Sum: %6" PRIu64" Ratio: %6.2f%%)\n", st.avgJumpEx(), st.maxJumpEx,
                   st.jumped(), st.jumpedRatio() * 100.0);
            printKeyValue("  Bounded", "%-8"
                    PRIu64, st.bounded);
            printf(" (Average: %5.2f Max: %3u Sum: %6" PRIu64" Ratio: %6.2f%%)\n", st.avgBound(), st.maxBound,
                   st.boundSum, 100.0 - (st.jumpedRatio() * 100.0));
        }
