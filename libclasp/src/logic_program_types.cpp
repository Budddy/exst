...
// auxB :- BODY.
        uint32 RuleTransform::transformChoiceRule(ProgramAdapter &prg, Rule &rule) const {
            uint32 newRules = 0;
            Var extraHead = ((rule.heads.size() * (rule.body.size() + 1)) + rule.heads.size()) >
                            (rule.heads.size() * 3) + rule.body.size()
                            ? prg.newAtom()
                            : varMax;
            Rule r1, r2;
            r1.setType(BASICRULE);
            r2.setType(BASICRULE);
            ...
            for (VarVec::iterator it = rule.heads.begin(), end = rule.heads.end(); it != end; ++it) {
                ...
                r2.heads.clear();
                Var aux = prg.newAtom();
                r1.heads.push_back(*it);
                r1.addToBody(aux, false, 1);
                r2.heads.push_back(aux);
                r2.addToBody(*it, false, 1);
        ...
        bool SccChecker::recurse(Call &c) {
            ...
            if (isNode(c.node, PrgEdge::BODY_NODE)) {
                ...
                for (PrgBody::head_iterator it = b->heads_begin() + c.next, end = b->heads_end(); it != end; ++it) {
                    if (it->isAtom()) {
                        h = prg_->getAtom(it->node());
                        t = PrgEdge::ATOM_NODE;
                    }
                    else {
                        h = prg_->getDisj(it->node());
                        t = PrgEdge::DISJ_NODE;
                    }
                ...
                for (PrgAtom::dep_iterator it = a->deps_begin() + c.next, end = a->deps_end(); it != end; ++it) {
                    ...
                    if (doVisit(bn, false) &&
                        onNode(bn, PrgEdge::BODY_NODE, c, static_cast<uint32>(it - a->deps_begin()))) {
                        return true;
                    }
                }
            }
...
/////////////////////////////////////////////////////////////////////////////////////////
        PrgHead::PrgHead(uint32 id, NodeType t, uint32 data, bool checkScc)
                : PrgNode(id, checkScc), data_(data), upper_(0), dirty_(0), freeze_(0),
                  isAtom_(t == PrgEdge::ATOM_NODE) {
            struct X {
                uint64 x;
                EdgeVec y;
                uint32 z;
...
/////////////////////////////////////////////////////////////////////////////////////////
        PrgBody::PrgBody(LogicProgram &prg, uint32 id, const BodyInfo &body, bool addDeps)
                : PrgNode(id, true), size_(body.size()), extHead_(0), type_(body.type()), sBody_(0), sHead_(0),
                  unsupp_(0) {
            ...
            for (WeightLitVec::const_iterator it = body.lits.begin(), end = body.lits.end(); it != end; ++it) {
                ...
                *p[x.sign()] = x;
                if (W) {
                    w = it->second;
                    data_.ext[0]->weights[p[x.sign()] - lits] = w;
                }
                ++p[x.sign()];
                sw[x.sign()] += w;
...
// body as a support for h.
        ...
        bool PrgBody::hasHead(PrgHead *h, EdgeType t) const {
            ...
            PrgEdge x = PrgEdge::newEdge(h->id(), t, h->isAtom() ? PrgEdge::ATOM_NODE : PrgEdge::DISJ_NODE);
            head_iterator it = sHead_ != 0 || !extHead() ? std::find(heads_begin(), heads_end(), x) : std::lower_bound(
                    heads_begin(), heads_end(), x);
...
// eqId   The id of a body in prg that is equivalent to this body	 
        ...
        bool PrgBody::normalize(const LogicProgram &prg, weight_t bound, weight_t sumW, weight_t reachW,
                                uint32 &hashOut) {
...
//  - it is a disjunction and one of the atoms is needed to satisfy the body
        bool PrgBody::superfluousHead(const LogicProgram &prg, const PrgHead *head, PrgEdge it,
                                      const RuleState &rs) const {
            if (it.isAtom()) {
                // the head is an atom
                ...
                return it.isChoice()
                       && (rs.inBody(negLit(atomId)) ||
                           rs.inHead(PrgEdge::newEdge(atomId, PrgEdge::NORMAL_EDGE, PrgEdge::ATOM_NODE)));
            }
            else {
                assert(it.isDisj());
                ...
                for (PrgDisj::atom_iterator aIt = dis->begin(), aEnd = dis->end(); aIt != aEnd; ++aIt) {
                    if (rs.inBody(posLit(aIt->node())) ||
                        rs.inHead(PrgEdge::newEdge(aIt->node(), PrgEdge::NORMAL_EDGE, PrgEdge::ATOM_NODE))) {
                        return true;
                    }
                ...
                if (prg.options().iters == LogicProgram::AspOptions::MAX_EQ_ITERS) {
                    for (head_iterator hIt = heads_begin(), hEnd = heads_end(); hIt != hEnd; ++hIt) {
                        if (hIt->isDisj() && prg.getDisj(hIt->node())->size() < dis->size()) {
                            ...
                            for (PrgDisj::atom_iterator a = other->begin(), aEnd = other->end();
                                 a != aEnd && other; ++a) {
...
// from TB follows conflict
        ...
        bool PrgBody::propagateAssigned(LogicProgram &prg, PrgHead *h, EdgeType t) {
            ...
            if (h->value() == value_false &&
                ...
                t == PrgEdge::NORMAL_EDGE) {
                return value() == value_false ||
                       (assignValue(value_false) && propagateValue(prg, prg.options().backprop));
            }
            return true;
        }
            ...
            if (sccMask != 0) {
                ...
                for (head_iterator h = heads_begin(), hEnd = heads_end(); h != hEnd; ++h) {
                    if (h->isAtom()) {
                        ...
                        PrgDisj *d = prg.getDisj(h->node());
                        aIt = d->begin(), aEnd = d->end();
                    }
                    for (; aIt != aEnd; ++aIt) {
                        atom = aIt->node();
