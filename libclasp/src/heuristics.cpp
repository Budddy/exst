...
#define BERK_MAX_DECAY 65534
    ...
    ClaspBerkmin::ClaspBerkmin(uint32 maxB, const HeuParams &params, bool berkHuang)
            : order_(berkHuang, params.resScore ? params.resScore : 3u), topConflict_(UINT32_MAX),
              topOther_(UINT32_MAX), front_(1), cacheSize_(5), numVsids_(0),
...
/////////////////////////////////////////////////////////////////////////////////////////
    ClaspVmtf::ClaspVmtf(LitVec::size_type mtf, const HeuParams &params) : decay_(0),
                                                                           MOVE_TO_FRONT(mtf >= 2 ? mtf : 2) {
    ...
    void ClaspVmtf::newConstraint(const Solver &s, const Literal *first, LitVec::size_type size, ConstraintType t) {
        if (t != Constraint_t::static_constraint) {
            ...
            const VarVec::size_type mtf =
                    t == Constraint_t::learnt_conflict ? MOVE_TO_FRONT : (MOVE_TO_FRONT * int(upAct)) / 2;
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    ClaspVsids_t<ScoreType>::ClaspVsids_t(double decay, const HeuParams &params)
            : vars_(CmpScore(score_)), decay_(1.0 / std::max(0.01, std::min(1.0, decay))), inc_(1.0) {
        scType_ = params.resScore ? params.resScore : 1u;
        uint32 x = params.otherScore + 1;
        if (x & Constraint_t::learnt_loop) { types_.addSet(Constraint_t::learnt_loop); }
        if (x & Constraint_t::learnt_other) { types_.addSet(Constraint_t::learnt_other); }
    ...
    template<class ScoreType>
    void ClaspVsids_t<ScoreType>::newConstraint(const Solver &, const Literal *first, LitVec::size_type size,
                                                ConstraintType t) {
        if (t != Constraint_t::static_constraint) {
            const bool upAct = types_.inSet(t);
            for (LitVec::size_type i = 0; i < size; ++i, ++first) {
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool DomEntry::isDomEntry(const SymbolType &sym) {
        return !sym.name.empty() && std::strncmp(sym.name.c_str(), domSym_s, domLen_s) == 0;
    }
    ...
    void DomEntry::init(const SymbolType &atomSym, const SymbolType &domSym) {
        ...
        // extract modifier
        if (std::strncmp(rule, "sign", 4) == 0) {
            this->mod = mod_sign;
            rule += 4;
        }
        else if (std::strncmp(rule, "true", 4) == 0) {
            this->mod = mod_tf;
            rule += 4;
            this->pref = trueValue(lit);
        }
        else if (std::strncmp(rule, "init", 4) == 0) {
            this->mod = mod_init;
            rule += 4;
        }
        else if (std::strncmp(rule, "level", 5) == 0) {
            this->mod = mod_level;
            rule += 5;
        }
        else if (std::strncmp(rule, "false", 5) == 0) {
            this->mod = mod_tf;
            rule += 5;
            this->pref = falseValue(lit);
        }
        else if (std::strncmp(rule, "factor", 6) == 0) {
            this->mod = mod_factor;
            rule += 6;
        }
    ...
    DomainHeuristic::DomainHeuristic(double d, const HeuParams &params) : ClaspVsids_t<DomScore>(d, params), domMin_(0),
                                                                          symSize_(0), defMod_(0), defPref_(0) {
    ...
    void DomainHeuristic::detach(Solver &s) {
        if (!actions_.empty()) {
            for (SymbolTable::const_iterator it = s.symbolTable().begin(), end = s.symbolTable().end();
                 it != end; ++it) {
    ...
    void DomainHeuristic::initScores(Solver &s, bool moms) {
        ...
        for (SymbolTable::const_iterator sIt = s.symbolTable().begin() + symSize_, end = s.symbolTable().end();
             sIt != end; ++sIt) {
        ...
        InitVec initVals;
        for (SymbolTable::const_iterator sIt = s.symbolTable().begin(), end = s.symbolTable().end();
             sIt != end && nRules; ++sIt) {
            if (sIt->second.name.empty() || s.topValue(sIt->second.lit.var()) != value_free) { continue; }
            const char *head = sIt->second.name.c_str();
            Var ev = 0;
            int16 init = 0;
            for (DomTable::const_iterator rIt = std::lower_bound(domTab.begin(), domTab.end(), head,
                                                                 DomEntry::Lookup());
                 rIt != domTab.end() && DomEntry::isHeadOf(head, *rIt); ++rIt, --nRules) {
                DomEntry e;
                ...
                int mods = 1;
                if (e.mod == DomEntry::mod_tf) {
                    e.mod = DomEntry::mod_level;
                    ++mods;
                }
                for (; ;) {
        ...
        if (defMod_) {
            MinimizeConstraint *m = s.enumerationConstraint()
                                    ? static_cast<EnumerationConstraint *>(s.enumerationConstraint())->minimizer() : 0;
            ...
            if ((prefSet & gpref_show) != 0) {
                for (SymbolTable::const_iterator it = s.symbolTable().begin() + symSize_, end = s.symbolTable().end();
                     it != end; ++it) {
            ...
            for (Var v = 1, end = s.numVars() + 1; v != end; ++v) {
                if (!prefSet && (s.varInfo(v).type() & Var_t::atom_var) != 0) {
                    addDefAction(s, posLit(v), 1, userKey);
                }
    ...
    void DomainHeuristic::applyAction(Solver &s, DomAction &a, uint16 &gPrio) {
        ...
        switch (a.mod) {
            default:
                assert(false);
                break;
            case DomEntry::mod_factor:
                std::swap(score_[a.var].factor, a.val);
                break;
