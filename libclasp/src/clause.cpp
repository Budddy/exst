...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    ClauseRep ClauseCreator::prepare(Solver &s, const Literal *in, uint32 inSize, const ClauseInfo &e, uint32 flags,
                                     Literal *out, uint32 outMax) {
        assert(out && outMax > 2);
    ...
    ClauseCreator::Status ClauseCreator::status(const Solver &s, const Literal *clause_begin,
                                                const Literal *clause_end) {
        if (clause_end <= clause_begin) { return status_empty; }
        Literal temp[3];
        ClauseRep x = prepare(const_cast<Solver &>(s), clause_begin, uint32(clause_end - clause_begin), ClauseInfo(), 0,
                              temp, 3);
        return status(s, x);
    }
    ...
    bool ClauseCreator::ignoreClause(const Solver &s, const ClauseRep &c, Status st, uint32 flags) {
        ...
        return st == status_subsumed || (st == status_sat && ((flags & clause_not_sat) != 0 ||
                                                              ((flags & clause_not_root_sat) != 0 &&
    ...
    ClauseHead *ClauseCreator::newProblemClause(Solver &s, const ClauseRep &clause, uint32 flags) {
        ...
        if (clause.size <= Clause::MAX_SHORT_LEN || !s.sharedContext()->physicalShareProblem()) {
            ...
            ret = Clause::newShared(s, SharedLiterals::newShareable(clause.lits, clause.size, clause.info.type(), 1),
                                    clause.info, clause.lits, false);
        }
    ...
    ClauseCreator::Result ClauseCreator::create_prepared(Solver &s, const ClauseRep &clause, uint32 flags) {
        ...
        if (clause.size > 1) {
            ...
            if ((flags & clause_no_heuristic) == 0) {
                s.heuristic()->newConstraint(s, clause.lits, clause.size, clause.info.type());
            }
            if (clause.size > 3 || (flags & clause_explicit) != 0 || !s.allowImplicit(clause)) {
                ret.local = clause.info.learnt() ? newLearntClause(s, clause, flags) : newProblemClause(s, clause,
                                                                                                        flags);
            }
            else {
            ...
            if ((x & (status_unit | status_unsat)) != 0) {
                ...
                if (!ret.local) {
                    ante = clause.size == 3 ? Antecedent(~clause.lits[1], ~clause.lits[2]) : Antecedent(
                            ~clause.lits[1]);
                }
    ...
    ClauseCreator::Result ClauseCreator::integrate(Solver &s, SharedLiterals *clause, uint32 modeFlags,
        ...
        Result result(0, xs);
        if ((modeFlags & clause_no_heuristic) == 0) {
        ...
        if (x.size > Clause::MAX_SHORT_LEN && s.sharedContext()->physicalShare(t)) {
            ...
            result.local =
                    x.size <= Clause::MAX_SHORT_LEN ? Clause::newClause(s, x) : newUnshared(s, clause, temp, x.info);
        }
        else {
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    ClauseHead *Clause::newShared(Solver &s, SharedLiterals *shared_lits, const ClauseInfo &e, const Literal *lits,
                                  bool addRef) {
    ...
    Clause::Clause(Solver &s, const ClauseRep &rep, uint32 tail, bool extend)
        ...
        if (!isSmall()) {
            ...
            data_.lits[0] =
                    rep.size > ClauseHead::HEAD_LITS ? rep.lits[ClauseHead::HEAD_LITS].asUint() : negLit(0).asUint();
            data_.lits[1] =
                    rep.size > ClauseHead::HEAD_LITS + 1 ? rep.lits[ClauseHead::HEAD_LITS + 1].asUint() : negLit(
                            0).asUint();
    ...
    Clause::Clause(Solver &s, const Clause &other) : ClauseHead(ClauseInfo()) {
        ...
        if (!isSmall()) { std::memcpy(head_, other.head_, oSize * sizeof(Literal)); }
            ...
            std::memcpy(data_.lits, other.data_.lits, (ClauseHead::MAX_SHORT_LEN + 1) * sizeof(Literal));
        }
    ...
    bool Clause::updateWatch(Solver &s, uint32 pos) {
        uint32 *n;
        if (!isSmall()) {
            for (Literal *begin = head_ + ClauseHead::HEAD_LITS, *end = longEnd(), *first =
                    begin + data_.local.idx; ;) {
...
/////////////////////////////////////////////////////////////////////////////////////////
    namespace mt {
        ClauseHead *SharedLitsClause::newClause(Solver &s, SharedLiterals *shared_lits, const ClauseInfo &e,
                                                const Literal *lits, bool addRef) {
            return new(s.allocSmall()) SharedLitsClause(s, shared_lits, lits, e, addRef);
        }

        SharedLitsClause::SharedLitsClause(Solver &s, SharedLiterals *shared_lits, const Literal *w,
                                           const ClauseInfo &e, bool addRef)
                : ClauseHead(e) {
...
// LoopFormula
/////////////////////////////////////////////////////////////////////////////////////////
    LoopFormula *LoopFormula::newLoopFormula(Solver &s, const ClauseRep &c1, const Literal *atoms, uint32 nAtoms,
                                             bool heu) {
        ...
        for (int bounds = 0, dir = ((data & 1) << 1) - 1; ;) {
            ...
            if (!isSentinel(*w)) {
                ...
                if (other_ == xPos_ && ok) { // all lits in inactive part are implied
                    for (Literal *it = xBegin(), *end = xEnd();
                         it != end && (ok = s.force(*it, this)) == true; ++it) { ; }
                }
    ...
    uint32 LoopFormula::isOpen(const Solver &s, const TypeSet &xs, LitVec &freeLits) {
        ...
        for (Literal *it = begin() + xPos_; !isSentinel(*it); ++it) {
            if (s.value(it->var()) == value_free) { freeLits.push_back(*it); }
                ...
                other_ = static_cast<uint32>(it - lits_);
                return 0;
            }
        }
        for (Literal *it = xBegin(), *end = xEnd(); it != end; ++it) {
