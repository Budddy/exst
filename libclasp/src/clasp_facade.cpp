...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    void ClaspConfig::prepare(SharedContext &ctx) {
        ...
        if (numS > solve.recommendedSolvers()) {
            ctx.report(warning(Event::subsystem_facade,
                               clasp_format_error("Oversubscription: #Threads=%u exceeds logical CPUs=%u.", numS,
                                                  solve.recommendedSolvers())));
        }
    ...
    bool ClaspConfig::addPost(Solver &s) const {
        ...
        if (s.sharedContext() && s.sharedContext()->sccGraph.get()) {
            DefaultUnfoundedCheck *pp = static_cast<DefaultUnfoundedCheck *>(s.getPost(
                    PostPropagator::priority_reserved_ufs));
            if (pp) {
                pp->setReasonStrategy(static_cast<DefaultUnfoundedCheck::ReasonStrategy>(solver(s.id()).loopRep));
            }
            else {
                ok = s.addPost(new DefaultUnfoundedCheck(
                        static_cast<DefaultUnfoundedCheck::ReasonStrategy>(solver(s.id()).loopRep)));
            }
        }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        enum State {
            state_start = 0, state_running = 1, state_result = 2, state_model = 3, state_done = 6
        };
        SolveStrategy() {
            state = signal = 0;
            algo = 0;
    ...
    struct ClaspFacade::SolveData {
        ...
        void reset() {
            if (active) {
                interrupt(SolveStrategy::SIGCANCEL);
                active->release();
                active = 0;
            }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    void ClaspFacade::init(ClaspConfig &config, bool discard) {
        ...
        if (config_->solve.enumMode == EnumOptions::enum_dom_record &&
            config_->solver(0).heuId != Heuristic_t::heu_domain) {
...
#define COMMON_KEYS "ctx.\0solvers.\0solver.\0costs.\0time_total\0time_cpu\0time_solve\0time_unsat\0time_sat\0enumerated\0optimal\0step\0result\0"
        static const char *_keys[] = {"accu.\0hccs.\0hcc.\0lp.\0" COMMON_KEYS, 0, "accu.\0lp.\0" COMMON_KEYS,
                                      "accu.\0" COMMON_KEYS};
        enum ObjId {
            id_hccs,
            id_hcc,
                ...
                switch (oId) {
                    default: {
                        ...
                        if (oId == id_costs) {
                            ...
                            if (x == path) {
                                if (!*path) {
                                    return keys ? ExpectedQuantity("__len\0")
                                                : ExpectedQuantity::error_ambiguous_quantity;
                                }
                    ...
                    case id_lp:
                        if (!lpStats_.get()) { return ExpectedQuantity::error_not_available; }
                        return keys ? GET_KEYS((*lpStats_), path) : GET_OBJ((*lpStats_), path);
                    case id_ctx:
                        return keys ? GET_KEYS(ctx.stats(), path) : GET_OBJ(ctx.stats(), path);
                    case id_solvers:
                        return keys ? GET_KEYS(ctx.stats(*ctx.solver(0), accu), path) : getStat(ctx, path, accu,
                                                                                                Range32(0,
                                                                                                        ctx.concurrency()));
                    case id_hccs:
                        if (!ctx.sccGraph.get() ||
                            ctx.sccGraph->numNonHcfs() == 0) { return ExpectedQuantity::error_not_available; } else {
                            ExpectedQuantity res(0.0);
                            for (SharedDependencyGraph::NonHcfIter it = ctx.sccGraph->nonHcfBegin(), end = ctx.sccGraph->nonHcfEnd();
                                 it != end; ++it) {
                    ...
                    case id_hcc:
                        Range32 r(0,
                                  oId == id_solver ? ctx.concurrency() : ctx.sccGraph.get() ? ctx.sccGraph->numNonHcfs()
                                                                                            : 0);
                        char *x;
                        r.lo = (uint32) std::strtoul(path, &x, 10);
                        if (x == path) {
                            if (!*path) {
                                return keys ? ExpectedQuantity("__len\0") : ExpectedQuantity::error_ambiguous_quantity;
                            }
                        ...
                        return keys ? GET_KEYS(active->stats(*active->solver(r.lo), accu), path) : getStat(*active,
                                                                                                           path, accu,
                                                                                                           r);
                }
            }
...
#undef COMMON_KEYS
    ...
    const char *ClaspFacade::getKeys(const char *path) const {
        ExpectedQuantity x =
                config_ && step_.totalTime >= 0.0 ? getStatImpl(path, true) : ExpectedQuantity::error_not_available;
        if (x.valid()) { return (const char *) static_cast<uintp>(x.rep); }
        return x.error() == ExpectedQuantity::error_unknown_quantity ? 0 : "\0";
    }
    ExpectedQuantity ClaspFacade::getStat(const SharedContext &ctx, const char *key, bool accu,
                                          const Range<uint32> &r) const {
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    const char *ClaspFacade::Summary::consequences() const {
        ...
        if ((mt & CBConsequences::cautious_consequences) ==
            CBConsequences::cautious_consequences) { return "Cautious"; }
        return 0;
    }
