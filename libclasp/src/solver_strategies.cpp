    ...
    uint32 SolverParams::prepare() {
        ...
        if (heuId == Heuristic_t::heu_unit) {
            if (!Lookahead::isType(lookType)) {
                res |= 2;
                lookType = Lookahead::atom_lookahead;
            }
            lookOps = 0;
        }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    ScheduleStrategy::ScheduleStrategy(Type t, uint32 b, double up, uint32 lim)
        ...
        else if (t == luby_schedule && lim) {
            len = std::max(uint32(2), (static_cast<uint32>(std::pow(2.0, std::ceil(log(double(lim)) / log(2.0)))) - 1) *
                                      2);
        }
    }
    ...
    uint64 ScheduleStrategy::current() const {
        enum {
            t_add = ScheduleStrategy::arithmetic_schedule, t_luby = ScheduleStrategy::luby_schedule
        };
    ...
    uint32 SumQueue::restart(uint32 maxLBD, float limMax) {
        ...
        if (upCfl >= upForce) {
            ...
            if (avg >= 16000.0) {
                ...
                lim += 0.05f;
                upForce = std::max(uint32(16000), upForce - 10000);
            }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    uint32 ReduceParams::getBase(const SharedContext &ctx) const {
        uint32 st = strategy.estimate != ReduceStrategy::est_dynamic || ctx.isExtended() ? strategy.estimate
                                                                                         : (uint32) ReduceStrategy::est_num_constraints;
        switch (st) {
            default:
            ...
            case ReduceStrategy::est_con_complexity :
                return ctx.stats().complexity;
            case ReduceStrategy::est_num_constraints:
                return ctx.stats().numConstraints();
            case ReduceStrategy::est_num_vars       :
                return ctx.stats().vars;
        }
    }
...
/////////////////////////////////////////////////////////////////////////////////////////
    SolveParams::SolveParams()
            : randRuns(0u), randConf(0u), randProb(0.0f) {
    }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    void BasicSatConfig::prepare(SharedContext &ctx) {
        ...
        if ((warn & 1) != 0) {
            ctx.report(warning(Event::subsystem_facade, "Selected heuristic requires lookback strategy!"));
        }
        if ((warn & 2) != 0) {
            ctx.report(warning(Event::subsystem_facade, "Heuristic 'Unit' implies lookahead. Using atom."));
        }
        if ((warn & 4) != 0) {
            ctx.report(warning(Event::subsystem_facade, "Domain options require heuristic 'Domain'!"));
        }
    }
...
/////////////////////////////////////////////////////////////////////////////////////////
    DecisionHeuristic *Heuristic_t::create(const SolverParams &str) {
        CLASP_FAIL_IF(str.search != SolverStrategies::use_learning && Heuristic_t::isLookback(str.heuId),
        ...
        if (id == heu_berkmin) { heu = new ClaspBerkmin(heuParam, params, str.berkHuang != 0); }
            ...
            if (id == heu_domain) {
                static_cast<DomainHeuristic *>(heu)->setDefaultMod(
                        static_cast<DomainHeuristic::GlobalModifier>(str.domMod), str.domPref);
            }
        }
