...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    void ProgramBuilder::addMinLit(WeightLiteral lit) {
        if (!min_.get()) { min_ = new MinimizeBuilder(); }
        min_->addLit(0, lit);
    }
    void ProgramBuilder::addMinRule(const WeightLitVec &lits) {
        if (!min_.get()) { min_ = new MinimizeBuilder(); }
        min_->addRule(lits);
    }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool SatBuilder::addClause(LitVec &clause, wsum_t cw) {
        ...
        CLASP_ASSERT_CONTRACT_MSG(cw >= 0 && (cw <= std::numeric_limits<weight_t>::max() || cw == hardWeight_),
                                  "Clause weight out of bounds!");
        if (cw == 0 && maxSat_) { cw = 1; }
        if (cw == hardWeight_) {
            return ClauseCreator::create(*ctx()->master(), clause, Constraint_t::static_constraint).ok() &&
                   markAssigned();
        }
        else {
    ...
    bool PBBuilder::addConstraint(WeightLitVec &lits, weight_t bound, bool eq, weight_t cw) {
        ...
        if (cw > 0) { // soft constraint
            if (lits.size() != 1) {
                ...
                if (lits[0].second < 0) {
                    bound += (lits[0].second = -lits[0].second);
                    lits[0].first = ~lits[0].first;
                }
        ...
        return WeightConstraint::create(*ctx()->master(), posLit(eqVar), lits, bound,
                                        !eq ? 0 : WeightConstraint::create_eq_bound).ok();
    }
