...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    void ModelEnumerator::RecordFinder::doCommitModel(Enumerator &en, Solver &s) {
        ...
        solution.clear();
        const LitVec *dom =
                (ctx.projectOpts() & uint32(project_dom_lits)) != 0 ? &s.sharedContext()->symbolTable().domLits : 0;
        if (dom && dom->empty()) {
            if (en.lastModel().num == 0) {
                s.sharedContext()->report(warning(Event::subsystem_solve, "domRec ignored: no domain atoms found!"));
            }
            dom = 0;
        }
        if (ctx.trivial() && !dom) {
            ...
            for (LitVec::const_iterator it = dom->begin(), end = dom->end(); it != end; ++it) {
                if (s.isTrue(*it) &&
                    (!project || s.sharedContext()->varInfo(it->var()).project())) { solution.push_back(~*it); }
            }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool ModelEnumerator::BacktrackFinder::doUpdate(Solver &s) {
        if (hasModel()) {
            ...
            else if (rep.size == 1 || s.isFalse(
                    rep.lits[1])) { // The projection nogood is unit. Force the single remaining literal from the current DL.
                ok = s.force(rep.lits[0], this);
            }
            else if (!s.isTrue(
                    rep.lits[0])) { // Shorten the projection nogood by assuming one of its literals to false.
                uint32 f = static_cast<uint32>(std::stable_partition(rep.lits + 2, rep.lits + rep.size, std::not1(
                        std::bind1st(std::mem_fun(&Solver::isFalse), &s))) - rep.lits);
                Literal x =
                        (opts & ModelEnumerator::project_use_heuristic) != 0 ? s.heuristic()->selectRange(s, rep.lits,
                                                                                                          rep.lits + f)
                                                                             : rep.lits[0];
...
/////////////////////////////////////////////////////////////////////////////////////////
    ModelEnumerator::ModelEnumerator(Strategy st)
            : Enumerator(), project_(0), options_(st) {
    }
    ...
    EnumerationConstraint *ModelEnumerator::doInit(SharedContext &ctx, SharedMinimizeData *opt, int numModels) {
        ...
        if (optOne && project_.get()) {
            ...
            if (!trivial) {
                ctx.report(
                        warning(Event::subsystem_prepare, "Projection: Optimization may depend on enumeration order."));
            }
        }
        if (st == strategy_auto) {
            st = trivial || (project_.get() && ctx.concurrency() > 1) ? strategy_record : strategy_backtrack;
        }
