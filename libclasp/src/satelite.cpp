...
/////////////////////////////////////////////////////////////////////////////////////////
        SatElite::SatElite()
                : occurs_(0), elimHeap_(LessOccCost(occurs_)), qFront_(0), facts_(0) {
        }
        ...
        bool SatElite::doPreprocess() {
            ...
            for (uint32 i = 0, end = opts_->limIters ? opts_->limIters : UINT32_MAX;
                 queue_.size() + elimHeap_.size() > 0; ++i) {
        ...
        bool SatElite::backwardSubsume() {
            ...
            while (qFront_ != queue_.size()) {
                ...
                for (uint32 i = 0, end = cls.left_size(); i != end; ++i) {
                    ...
                    if (other && other != &c &&
                        (res = subsumes(c, *other, best.sign() == cl.sign() ? posLit(0) : best)) != negLit(0)) {
                        if (res == posLit(0)) {
...
//  - l         - The literal l can be deleted from 'other'
        Literal SatElite::subsumes(const Clause &c, const Clause &other, Literal res) const {
            ...
            if (c.size() < 10 || other.size() < 10) {
                ...
                for (uint32 i = 0; i != c.size(); ++i) {
                    if (occurs_[c[i].var()].litMark == 0) {
                        res = negLit(0);
                        break;
                    }
                    if (occurs_[c[i].var()].marked(!c[i].sign())) {
                        if (res != posLit(0) && res != c[i]) {
                            res = negLit(0);
                            break;
                        }
                        res = c[i];
