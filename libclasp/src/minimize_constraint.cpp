...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    MinimizeConstraint *SharedMinimizeData::attach(Solver &s, MinimizeMode_t::Strategy strat, uint32 param,
                                                   bool addRef) {
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    DefaultMinimize::DefaultMinimize(SharedData *d, uint32 strat)
            : MinimizeConstraint(d), bounds_(0), pos_(d->lits), undo_(0), undoTop_(0), size_(d->numRules()) {
        step_.type = strat & 3u;
...
// Disables the minimize constraint by clearing its upper bound.
    ...
    void DefaultMinimize::stepInit(uint32 n) {
        ...
        if (step_.type) {
            step_.lev = n;
            if (n != size_) stepLow() = 0 - shared_->maxBound();
        }
...
// previously added minimize statements
    MinimizeBuilder::SharedData *MinimizeBuilder::build(SharedContext &ctx) {
        ...
        SharedData *srep = new(::operator new(sizeof(SharedData) + (lits_.size() * sizeof(WeightLiteral)))) SharedData(
                adjust_);
        if (adjust_.size() == 1) {
            for (LitVec::size_type i = 0; i != lits_.size(); ++i) {
...
/////////////////////////////////////////////////////////////////////////////////////////
    UncoreMinimize::UncoreMinimize(SharedMinimizeData *d, uint32 strat)
            : MinimizeConstraint(d), enum_(0), sum_(new wsum_t[d->numRules()]), auxInit_(UINT32_MAX), auxAdd_(0),
              freeOpen_(0), options_(0) {
        options_ = strat & 7u;
    }
...
// the root path.
    bool UncoreMinimize::pushPath(Solver &s) {
        ...
        while (path) {
            ...
            CLASP_FAIL_IF(!sat_ && s.decisionLevel() != s.rootLevel(), "pushPath must be called on root level (%u:%u)",
        ...
        do {
            if (sat_ == 0) {
                ...
                for (LitSet::iterator it = todo_.begin(), end = todo_.end(), cs; (cs = it) != end; ++it) {
                    ...
                    while (it->id) {
                        w = std::min(w, getData(it->id).weight);
                        ++it;
                    }
                    lower_ -= w;
...
// Eliminates the given core by adding suitable constraints to the solver.
    ...
    bool UncoreMinimize::addCore(Solver &s, const WCTemp &wc, weight_t weight) {
        ...
        uint32 fset = WeightConstraint::create_explicit | WeightConstraint::create_no_add |
