...
#undef RETURN_IF
    ...
    const char *ProblemStats::keys(const char *k) {
        if (!k ||
            !*k) { return "vars\0vars_eliminated\0vars_frozen\0constraints\0constraints_binary\0constraints_ternary\0complexity\0"; }
        return 0;
    }
/////////////////////////////////////////////////////////////////////////////////////////
...
#undef FOR_EACH_LEARNT
    ...
    bool ShortImplicationsGraph::reverseArc(const Solver &s, Literal p, uint32 maxLev,
                                            Antecedent &out) const { return !forEach(p, ReverseArc(s, maxLev, out)); }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    void SatPreprocessor::Clause::strengthen(Literal p) {
        ...
        for (end = size_ - 1; i < end; ++i) {
            lits_[i] = lits_[i + 1];
            a |= Clause::abstractLit(lits_[i]);
        }
        --size_;
        data_.abstr = a;
    }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool SharedContext::ok() const {
        return master()->decisionLevel() || !master()->hasConflict() || master()->hasStopConflict();
    }
    ...
    bool SharedContext::endInit(bool attachAll) {
        ...
        SatPrePtr temp;
        satPrepro.swap(temp);
        bool ok = !master()->hasConflict() && master()->preparePost() && (!temp.get() || temp->preprocess(*this)) &&
                  master()->endInit();
        satPrepro.swap(temp);
    ...
    void SharedContext::simplify(bool shuffle) {
        ...
        if (concurrency() == 1 || master()->dbIdx_ == 0) {
            ...
            for (Solver::ConstraintDB::size_type i = 0, end = db.size(); i != end; ++i) {
                Constraint *c = db[i];
                if (c->simplify(*master(), shuffle)) {
                    c->destroy(master(), false);
                    db[i] = 0;
                    ++rem;
                }
            }
            if (rem) {
                for (SolverVec::size_type s = 1; s != solvers_.size(); ++s) {
                    ...
                    if (x.dbIdx_ == db.size()) { x.dbIdx_ -= rem; }
                        ...
                        x.dbIdx_ -= (uint32) std::count_if(db.begin(), db.begin() + x.dbIdx_, IsNull());
                    }
                }
