...
// - If the condition cannot be restored, the body is marked as invalid source.
    ...
    DefaultUnfoundedCheck::DefaultUnfoundedCheck(ReasonStrategy st)
            : solver_(0), graph_(0), mini_(0), reasons_(0), strategy_(st) {
        mini_.release();
    }
...
// creates & populates WS and adds watches to all subgoals
    void DefaultUnfoundedCheck::initExtBody(const BodyPtr &n) {
        ...
        ExtData *extra = new(::operator new(sizeof(ExtData) + (ExtData::flagSize(preds) * sizeof(uint32)))) ExtData(
                n.node->ext_bound(), preds);
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    bool DefaultUnfoundedCheck::simplify(Solver &s, bool) {
        if (!s.sharedContext()->isShared()) { graph_->simplify(s); }
            ...
            for (SharedDependencyGraph::NonHcfIter hcc = graph_->nonHcfBegin(), hccEnd = graph_->nonHcfEnd();
                 hcc != hccEnd; ++hcc) {
...
// n is no longer a valid source, forward propagate this information to its heads
    void DefaultUnfoundedCheck::forwardUnsource(const BodyPtr &n, bool add) {
        for (const NodeId *x = n.node->heads_begin();
             x != n.node->heads_end() && graph_->getAtom(*x).scc == n.node->scc; ++x) {
...
// falsifies the atoms one by one from the unfounded set stored in unfounded_
    bool DefaultUnfoundedCheck::falsifyUfs(UfsType t) {
        ...
        for (uint32 rDL = 0; !ufs_.empty();) {
            ...
            else if (solver_->decisionLevel() !=
                     rDL) { break; /* atoms may no longer be unfounded after backtracking */ }
        }
        if (!loopAtoms_.empty()) {
            createLoopFormula();
        }
...
// asserts an unfounded atom using the selected reason strategy
    bool DefaultUnfoundedCheck::assertAtom(Literal a, UfsType t) {
        ...
        bool noClause = solver_->isTrue(a) || strategy_ == no_reason || strategy_ == only_reason ||
                        (strategy_ == shared_reason && activeClause_.size() > 3);
        if (noClause) {
    ...
    void DefaultUnfoundedCheck::createLoopFormula() {
        ...
        if (loopAtoms_.size() == 1) {
            ...
            ante = LoopFormula::newLoopFormula(*solver_,
                                               ClauseRep::prepared(&activeClause_[0], (uint32) activeClause_.size(),
                                                                   info_), &loopAtoms_[0], (uint32) loopAtoms_.size());
            solver_->addLearnt(static_cast<LearntConstraint *>(ante.constraint()),
                               uint32(loopAtoms_.size() + activeClause_.size()), Constraint_t::learnt_loop);
        }
        do {
...
// computes the reason why a set of atoms is unfounded
    void DefaultUnfoundedCheck::computeReason(UfsType t) {
        ...
        uint32 rc = !solver_->isFalse(activeClause_[0]) && activeClause_.size() > 100 &&
    ...
    void DefaultUnfoundedCheck::addDeltaReason(const BodyPtr &n, uint32 uScc) {
        ...
        uint32 bodyAbst = solver_->isFalse(n.node->lit) ? solver_->level(n.node->lit.var()) : solver_->decisionLevel() +
                                                                                              1;
        for (const NodeId *x = n.node->heads_begin(); x != n.node->heads_end(); ++x) {
            if (*x != SharedDependencyGraph::sentinel_atom) { // normal head
                ...
                for (++x; *x; ++x) {
                    ...
                    else if (solver_->isTrue(aLit = graph_->getAtom(*x).lit) &&
                             solver_->level(aLit.var()) < reasonAbst) {
                        reasonLit = ~aLit;
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    DefaultUnfoundedCheck::MinimalityCheck::MinimalityCheck(const FwdCheck &afwd) : fwd(afwd), high(UINT32_MAX), low(0),
                                                                                    next(0), scc(0) {
