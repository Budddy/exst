...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    void EnumerationConstraint::init(Solver &s, SharedMinimizeData *m, QueuePtr p) {
        ...
        queue_ = p;
        if (m) {
            const SolverParams *c = s.sharedContext()->configuration() ? &s.sharedContext()->configuration()->solver(
                    s.id()) : 0;
            MinimizeMode_t::Strategy st = c ? static_cast<MinimizeMode_t::Strategy>(c->optStrat)
                                            : MinimizeMode_t::opt_bb;
    ...
    bool EnumerationConstraint::integrateNogoods(Solver &s) {
        ...
        const uint32 f =
                ClauseCreator::clause_no_add | ClauseCreator::clause_no_release | ClauseCreator::clause_explicit;
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    int  Enumerator::init(SharedContext &ctx, SharedMinimizeData *min, int limit) {
        ...
        reset();
        if (min && min->mode() == MinimizeMode_t::ignore) {
            min->release();
            min = 0;
        }
        mini_ = min;
        ...
        else if (cons && optimize()) {
            ctx.report(
                    warning(Event::subsystem_prepare, "Optimization: Consequences may depend on enumeration order."));
        }
    ...
    ValueRep Enumerator::commit(Solver &s) {
        ...
        else if (s.numFreeVars() == 0 && s.queueSize() == 0 && !s.hasConflict()) {
    ...
    bool Enumerator::supportsSplitting(const SharedContext &ctx) const {
        ...
        for (uint32 i = 0; i != ctx.concurrency() && ok; ++i) {
            if (ctx.hasSolver(i) && constraint(*ctx.solver(i))) {
                ok = constraint(*ctx.solver(i))->minimizer()->supportsSplitting();
            }
            else if (config && i < config->numSolver()) {
                ok = MinimizeMode_t::supportsSplitting(
                        static_cast<MinimizeMode_t::Strategy>(config->solver(i).optStrat));
            }
        }
        return ok;
    }
