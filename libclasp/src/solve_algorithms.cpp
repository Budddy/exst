...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    BasicSolve::BasicSolve(Solver &s, SolveLimits *lim) : solver_(&s), params_(&s.searchConfig()), limits_(lim),
                                                          state_(0) { }
    BasicSolve::BasicSolve(Solver &s, const SolveParams &p, SolveLimits *lim)
            : solver_(&s), params_(&p), limits_(lim), state_(0) {
    }
    ...
    ValueRep BasicSolve::State::solve(Solver &s, const SolveParams &p, SolveLimits *lim) {
        ...
        ConflictLimits cLimit = {UINT64_MAX, dbRed.current() + dbRedInit, dbGrowNext,
                                 lim ? lim->conflicts : UINT64_MAX};
        ...
        for (EventType progress(s, EventType::event_restart, 0, 0); cLimit.global;) {
            ...
            if (progress.op) {
                ...
                progress.op = (uint32) EventType::event_none;
            }
            ...
            if (cLimit.grow == 0 || (dbGrow.defaulted() && progress.op == (uint32) EventType::event_restart)) {
                ...
                if (cLimit.grow == 0) {
                    cLimit.grow = dbGrow.next();
                    minLimit = cLimit.grow;
                    ++nGrow;
                }
                if ((s.numLearntConstraints() + minLimit) > dbMax) {
                    dbMax *= p.reduce.fGrow;
                    progress.op = std::max(progress.op, (uint32) EventType::event_grow);
                }
                if (dbMax > dbHigh) {
                    dbMax = dbHigh;
...
// SolveAlgorithm
/////////////////////////////////////////////////////////////////////////////////////////
    SolveAlgorithm::SolveAlgorithm(Enumerator *e, const SolveLimits &lim) : limits_(lim), enum_(e), onModel_(0),
    ...
    bool SolveAlgorithm::solve(SharedContext &ctx, const LitVec &assume, ModelHandler *onModel) {
        struct Scoped {
            ...
            bool solve(const LitVec &assume, ModelHandler *h) {
                ...
                if (!algo->enum_) {
                    temp = EnumOptions::nullEnumerator();
                    algo->setEnumerator(*temp);
                }
                algo->onModel_ = h;
                if (algo->maxModels() != UINT64_MAX) {
                    if (algo->enum_->optimize() && !algo->enum_->tentative()) {
                        ctx->report(warning(Event::subsystem_solve,
                ...
                //TODO
                //exst::StatsCalculator::getInstance().labelGraph(ctx->symbolTable());
    ...
    bool SolveAlgorithm::moreModels(const Solver &s) const {
        return s.decisionLevel() != 0 || !s.symmetric().empty() ||
               (!s.sharedContext()->preserveModels() && s.sharedContext()->numEliminatedVars());
    }
...
/////////////////////////////////////////////////////////////////////////////////////////
    ...
    SequentialSolve::SequentialSolve(Enumerator *enumerator, const SolveLimits &limit)
            : SolveAlgorithm(enumerator, limit), term_(0) {
    }
