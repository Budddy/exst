            ...
            struct LessBodySize {
                ...
                bool operator()(Var b1, Var b2) const {
                    return (*bodies_)[b1]->size() < (*bodies_)[b2]->size()
                           || ((*bodies_)[b1]->size() == (*bodies_)[b2]->size() &&
                               (*bodies_)[b1]->type() < (*bodies_)[b2]->type());
                }
            private:
...
// Adds nogoods representing this node to the solver.
        ...
        void LogicProgram::setOptions(const AspOptions &opts) {
            ...
            if (opts.suppMod) {
                if (!incData_ || incData_->startScc == 0) {
                    if (opts_.iters && ctx()) {
                        ctx()->report(warning(Event::subsystem_prepare, "'supp-models' implies 'eq=0'."));
                    }
                    ...
                    if (ctx()) {
                        ctx()->report(
                                warning(Event::subsystem_facade, "'supp-models' ignored for non-tight programs."));
                    }
                    opts_.suppMod = 0;
                    opts_.noSCC = 0;
                }
        ...
        bool LogicProgram::doUpdateProgram() {
            if (!incData_) {
                incData_ = new Incremental();
                ctx()->symbolTable().incremental(true);
            }
                        ...
                        for (PrgBody::head_iterator hIt = b->heads_begin(); hIt != b->heads_end(); ++hIt) {
                            ...
                            if (hIt->isAtom()) {
                                if (hIt->isNormal()) {
                                    ...
                                    if (activeBody_.type() != BodyInfo::NORMAL_BODY &&
                                        getHead(*hIt)->literal() == b->literal()) {
                                        activeBody_.reset();
...
#define check_modular(x, atomId) (void)( (!!(x)) || (throw RedefinitionError((atomId), this->getAtomName((atomId))), 0))
        RedefinitionError::RedefinitionError(unsigned atomId, const char *name) : std::logic_error(
                clasp_format_error("Program not modular: Redefinition of atom <%u,'%s'>", atomId, name)) {
        }
...
/////////////////////////////////////////////////////////////////////////////////////////
        ...
        void LogicProgram::doGetAssumptions(LitVec &out) const {
            if (incData_) {
                for (VarVec::const_iterator it = incData_->frozen.begin(), end = incData_->frozen.end();
                     it != end; ++it) {
...
/////////////////////////////////////////////////////////////////////////////////////////
        ...
        bool LogicProgram::transformNoAux(RuleType r, const BodyInfo &body) const {
            return r != CHOICERULE &&
                   (body.bound() == 1 || (body.size() <= 6 && choose(body.size(), body.bound()) <= 15));
        }
        ...
        void LogicProgram::transformIntegrity(uint32 nAtoms, uint32 maxAux) {
            ...
            if (!integrity.empty() && (integrity.size() == 1 || (nAtoms / double(bodies_.size()) > 0.5 &&
                                                                 integrity.size() / double(bodies_.size()) < 0.01))) {
...
// replace equivalent atoms in minimize rules
        ...
        void LogicProgram::prepareProgram(bool checkSccs) {
            ...
            if (hasConflict() || !propagate(true) ||
                !p.preprocess(*this, opts_.iters != 0 && !opts_.suppMod ? Preprocessor::full_eq : Preprocessor::no_eq,
                              opts_.iters, opts_.dfOrder)) {
                setConflict();
                return;
            }
...
// replace disjunctions with gamma (shifted) and delta (component-shifted) rules
        void LogicProgram::finalizeDisjunctions(Preprocessor &p, uint32 numSccs) {
            ...
            enum SccFlag {
                seen_scc = 1u, is_scc_non_hcf = 128u
            };
                    ...
                    if (scc == PrgNode::noScc || (sccMap[scc] & seen_scc) != 0) {
                        ...
                        if (t == BASICRULE) {
                            ...
                            if (!options().noGamma) {
                                ...
                                for (uint32 i = 1; i != rule_.heads.size(); ++i) {
                                    rule_.addToBody(rule_.heads[i], false);
                                }
                                ...
                                for (uint32 i = x->size(); ;) {
                                    ...
                                    if (B && B->value() != value_false &&
                                        !B->hasHead(getAtom(activeHead_[0]), PrgEdge::NORMAL_EDGE)) {
...
// add (completion) nogoods
        bool LogicProgram::addConstraints() {
            ...
            CLASP_ASSERT_CONTRACT(ctx()->symbolTable().curBegin() == ctx()->symbolTable().end() ||
                                  startAtom() <= ctx()->symbolTable().curBegin()->first);
            ...
            for (WeightLitVec::const_iterator it = r.body.begin(), bEnd = r.body.end(); it != bEnd; ++it) {
                ...
                if (a->value() != value_free || !a->relevant()) {
                    ...
                    if (vSign != lit.sign()) {
                        ...
                        if ((bound -= w) <= 0) {
                            while (!sBody.empty()) {
                                ruleState_.clear(sBody.back().first.var());
                                sBody.pop_back();
                            }
                            pos = hash = 0;
                            break;
...
// aux == S1 v ... v Sn
        Literal LogicProgram::getEqAtomLit(Literal lit, const BodyList &supports, Preprocessor &p,
                                           const SccMap &sccMap) {
        ...
        uint32 LogicProgram::equalBody(const IndexRange &range, BodyInfo &body) const {
            ...
            for (IndexIter it = range.first; it != range.second; ++it) {
                ...
                if (o.type() == body.type() && o.size() == body.size() && o.bound() == body.bound() &&
                    (body.posSize() == 0u || o.goal(body.posSize() - 1).sign() == false)) {
        ...
        PrgDisj *LogicProgram::getDisjFor(const VarVec &heads, uint32 headHash) {
            ...
            if (headHash) {
                ...
                for (; eqRange.first != eqRange.second; ++eqRange.first) {
                    ...
                    if (o.relevant() && o.size() == heads.size() &&
                        ruleState_.allMarked(o.begin(), o.end(), RuleState::head_flag)) {
...
// returns whether posSize(root) <= posSize(body)
        ...
        PrgBody *LogicProgram::mergeEqBodies(PrgBody *b, Var rootId, bool hashEq, bool atomsAssigned) {
            ...
            if (!b->simplifyHeads(*this, atomsAssigned) || (b->value() != root->value() &&
                                                            (!mergeValue(b, root) || !root->propagateValue(*this, bp) ||
                                                             !b->propagateValue(*this, bp)))) {
                setConflict();
                return 0;
            }
