...
/////////////////////////////////////////////////////////////////////////////////////////
namespace bk_lib {
    static const struct OffType {
    } off = {};

    template<class T, class U>
    struct Or {
        Or(T &l, U &r) : lhs(&l), rhs(&r) { }
        T *lhs;
        U *rhs;
    };

    template<class T>
    static Or<T, const OffType> operator|(T &lhs, const OffType &) { return Or<T, const OffType>(lhs, off); }
    template<class T>
    static Or<const OffType, T> operator|(const OffType &, T &rhs) { return Or<const OffType, T>(off, rhs); }
    ...
    template<class X1, class X2 = void, class X3 = void>
    struct Arg_t;
    ...
    struct Arg_t : Arg_t<X1, X2, void> {
        ...
        int parse(const char *n, const char **next) {
            ...
            return this->parsed_ = (ret == 2 ? 2 + int(*(n = *next) == ',' && *++n && xconvert(n, third, next, 0) > 0)
                                             : ret);
        }
        X3 third;
    };
    ...
    static std::size_t findEnumValImpl(const char *value, int &out, const char *k1, int v1, va_list args) {
        ...
        if (vLen == kLen && strncasecmp(value, k1, kLen) == 0) {
            out = v1;
            return kLen;
        }
        while (const char *key = va_arg(args, const char *)) {
            ...
            kLen = std::strlen(key);
            if (vLen == kLen && strncasecmp(value, key, kLen) == 0) {
                out = val;
                return kLen;
            }
        }
    ...
    template<class T, class U>
    static bool stringTo(const char *str, const Or<T, U> &x) {
        return bk_lib::stringTo(str, *x.lhs) || bk_lib::stringTo(str, *x.rhs);
    }
}
namespace Clasp {
/////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////
    static int xconvert(const char *x, ScheduleStrategy &out, const char **errPos, int e) {
        ...
        if (strncasecmp(x, "f,", 2) == 0 || strncasecmp(x, "fixed,", 6) == 0) {
            ...
            if (strncasecmp(x, "d", 1) == 0 && arg.first > 0.0) {
                out = ScheduleStrategy(ScheduleStrategy::user_schedule, base, arg.first, arg.second);
            }
            else if (strncasecmp(x, "d", 1) != 0 && arg.first >= 1.0) {
    ...
    static std::string &xconvert(std::string &out, const ScheduleStrategy &sched) {
        ...
        switch (sched.type) {
            ...
            case ScheduleStrategy::arithmetic_schedule:
                if (sched.grow) {
                    out[t] = '+';
                    return xconvert(out.append(1, ','), std::make_pair((uint32) sched.grow, sched.len));
                }
                else {
                    out[t] = 'f';
                    return out;
            ...
            case ScheduleStrategy::user_schedule:
                ...
                return xconvert(out.append(1, ','), std::make_pair((double) sched.grow, sched.len));
            default:
                CLASP_FAIL_IF(true, "xconvert(ScheduleStrategy): unknown type");
        }
    }
    static int xconvert(const char *x, SatPreParams &out, const char **errPos, int) {
        ...
        if (!nt || (arg[0] > 3u && arg[0] != UINT32_MAX) || (nt > 1 && !arg[0]) ||
            (nt > 4 && arg[4] > 100u)) { nt = 0; }
        switch (nt) {
            case 6:
                SET_OR_ZERO(out.limClause, arg[5]);
            case 5:
                SET_OR_ZERO(out.limFrozen, arg[4]);
            case 4:
                SET_OR_ZERO(out.limTime, arg[3]);
            case 3:
                SET_OR_ZERO(out.limOcc, arg[2]);
            case 2:
                SET_OR_ZERO(out.limIters, arg[1]);
            case 1:
                out.type = arg[0];
            default:
                if (errPos && nt) { *errPos = x; }
                return nt;
        }
    }
...
#undef GRP
        ...
        static inline bool isTesterOption(int k) {
            return k >= option_category_nodes_end && k < option_category_search_end;
        }
        static inline bool isSolverOption(int k) {
...
"        <file>: Use configuration file to configure solver(s)"
        ...
        enum {
            key_leaf = 0, key_solver = -1, key_asp = -2, key_solve = -3, key_tester = -4, key_root = -5
        };
...
/* 5: */
        ...
        const ClaspCliConfig::KeyType ClaspCliConfig::KEY_TESTER = makeKeyHandle(key_tester,
                                                                                 ClaspCliConfig::mode_tester, 0);
...
// Type for storing one command-line option.
        ...
        public:
            ...
            bool doParse(const std::string &opt, const std::string &value) {
                int ret = isOption(option_) ? config_->setActive(option_, value.c_str()) : config_->setAppOpt(option_,
                                                                                                              value.c_str());
                if (ret == -1) {
                    throw ProgramOptions::UnknownOption(config_->isGenerator() ? "<clasp>" : "<tester>", opt);
                }
                return ret > 0;
            }
...
// Adapter for parsing a command string.
        ...
        void ClaspCliConfig::ParseContext::addValue(const OptPtr &key, const std::string &value) {
            ...
            if (exclude->count(key->name()) == 0) {
                ...
                if ((xs & m) != 0 && !v->isComposing()) {
                    throw ValueError(config, ValueError::multiple_occurences, key->name(), value);
                }
                if (!v->parse(key->name(), value, s)) {
                    throw ValueError(config, ValueError::invalid_value, key->name(), value);
                }
...
#include <clasp/cli/clasp_cli_configs.inl>
                ...
                case config_default:
                    return ConfigIter("/default\0/\0");
                default            :
                    throw std::logic_error(clasp_format_error("Invalid config key '%d'", (int) k));
            }
        }
...
/////////////////////////////////////////////////////////////////////////////////////////
        ...
        void ClaspCliConfig::createOptions() {
            ...
            opts_->addOptions()("configuration",
                                ...
                                KEY_INIT_DESC("Configure default configuration [%D]\n"));
            std::string cmdName;
#define CLASP_CONTEXT_OPTIONS
#define CLASP_SOLVE_OPTIONS
#define CLASP_ASP_OPTIONS
...
#include <clasp/cli/clasp_cli_options.inl>
        ...
        bool ClaspCliConfig::assignDefaults(const ProgramOptions::ParsedOptions &exclude) {
            for (Options::option_iterator it = opts_->begin(), end = opts_->end(); it != end; ++it) {
                ...
                CLASP_FAIL_IF(exclude.count(o.name()) == 0 && !o.assignDefault(),
                              "Option '%s': invalid default value '%s'\n", o.name().c_str(), o.value()->defaultsTo());
            }
            return true;
        }
        ...
        ClaspCliConfig::KeyType ClaspCliConfig::getArrKey(KeyType k, unsigned i) const {
            ...
            if (id != key_solver || (decodeMode(k) & mode_solver) != 0 ||
                i >= solve.supportedSolvers()) { return INVALID_KEY; }
        ...
        int ClaspCliConfig::getKeyInfo(KeyType k, int *nSubkeys, int *arrLen, const char **help, int *nValues) const {
            ...
            if (isLeafId(id)) {
                ...
                if (nValues && ++ret) {
                    *nValues = static_cast<int>((decodeMode(k) & mode_tester) == 0 || testerConfig() != 0 );
                }
        ...
        int ClaspCliConfig::getValue(KeyType key, std::string &out) const {
            ...
            try {
                int ret = ScopedSet(const_cast<ClaspCliConfig &>(*this), decodeMode(key), decodeSolver(key))->getActive(
                        id, &out, 0, 0);
        ...
        int ClaspCliConfig::applyActive(int o, const char *_val_, std::string *_val_out_, const char **_desc_out_,
                                        const char **_name_out_) {
            ...
            if (_val_ || _val_out_) {
                if (!active || (active == testerConfig() && !isTesterOption(o)) ||
                    ((cliMode & mode_solver) != 0 && !isSolverOption(o))) {
...
#undef ARG_T
        ...
        int ClaspCliConfig::setAppOpt(int o, const char *_val_) {
            if (o == meta_config) {
                ConfigKey defC;
                if (bk_lib::stringTo(_val_, defC)) {
                    active()->cliConfig = (uint8) defC;
                    return true;
                }
                else {
                    CLASP_FAIL_IF(!std::ifstream(_val_).is_open(), "Could not open config file '%s'", _val_);
                    config_[!isGenerator()] = _val_;
                ...
                return ret &&
                       finalizeAppConfig(testerConfig(), finalizeParsed(testerConfig(), ex, ex), Problem_t::ASP, true);
            }
        ...
        bool ClaspCliConfig::setConfig(const ConfigIter &config, bool allowMeta, const ParsedOpts &exclude,
                                       ParsedOpts *out) {
            createOptions();
        ...
        bool ClaspCliConfig::validate() {
            ...
            do {
                for (uint32 i = 0; i != (*c)->numSolver(); ++i) {
                    Clasp::Cli::validate(clasp_format(ctx, 80, "<%s>.%u", *c == this ? "<config>" : "<tester>", i),
                                         (*c)->solver(i), (*c)->search(i));
                }
            } while (*++c);
            return true;
        }
        ...
        const ClaspCliConfig::ParsedOpts &ClaspCliConfig::finalizeParsed(UserConfig *active, const ParsedOpts &parsed,
        ...
        bool ClaspCliConfig::finalizeAppConfig(UserConfig *active, const ParsedOpts &parsed, ProblemType t, bool defs) {
            ...
            if (c == config_default) {
                ...
                else if (solve.numSolver() == 1 || !solve.defaultPortfolio()) {
                    c = t == Problem_t::ASP ? (uint8) config_asp_default : (uint8) config_sat_default;
                }
                else { c = config_many; }
            }
            std::string m;
        ...
        bool ClaspCliConfig::setAppConfig(const RawConfig &config, ProblemType t) {
            ...
            reset();
            return setConfig(config.iterator(), true, exclude, &exclude) && assignDefaults(exclude) &&
        ...
        void validate(const char *ctx, const SolverParams &solver, const SolveParams &search) {
            ...
            if (solver.search == SolverParams::no_learning) {
                CLASP_FAIL_IF(Heuristic_t::isLookback(solver.heuId), "'%s': Heuristic requires lookback strategy!",
                              ctx);
                CLASP_FAIL_IF(!search.restart.sched.disabled() && !search.restart.sched.defaulted(),
                              "'%s': 'no-lookback': restart options disabled!", ctx);
                CLASP_FAIL_IF(!reduce.cflSched.disabled() ||
                              (!reduce.growSched.disabled() && !reduce.growSched.defaulted()) ||
                              search.reduce.fReduce() != 0, "'%s': 'no-lookback': deletion options disabled!", ctx);
            }
            bool hasSched =
                    !reduce.cflSched.disabled() || !reduce.growSched.disabled() || reduce.maxRange != UINT32_MAX;
            CLASP_FAIL_IF(hasSched && reduce.fReduce() == 0.0f && !reduce.growSched.defaulted(),
                          "'%s': 'no-deletion': deletion strategies disabled!", ctx);
            CLASP_FAIL_IF(!hasSched && reduce.fReduce() != 0.0f && !reduce.growSched.defaulted(),
                          "'%s': 'deletion': deletion strategy required!", ctx);
        }
    }
}
