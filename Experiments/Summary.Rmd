...
kable(overview[order(as.numeric(overview[, 2]), decreasing = TRUE),], digits=3, col.names = c("Problem","Number of Instances"))
# kable(su, digits=3, col.names = c("Problem","Statistic","Min","Max","Mean","Median"))

```

## Number of instances per problem alphabetical
```{r Summary per problem alphabetical, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
kable(overview[order(overview[, 1], decreasing = FALSE),], digits=3, col.names = c("Problem","Number of Instances"))
# kable(su, digits=3, col.names = c("Problem","Statistic","Min","Max","Mean","Median"))
...
for (i in 3:len) {
  ...
  for (pro in 1:numprobs) {
    if (numvalues[pro, i] >= mininst) {
      ...
      a <-
        cor.test(valmatrix[[pro]][[i]], valmatrix[[pro]]$CPU_time, method = "spearman")
  ...
  if (numhigh >= 9) {
    cat(sprintf("\n\n### Time vs. %s \n\n", columns[i]))
    cat(sprintf("\n\nhigh: %i, low: %i mean: %.2f \n\n", numhigh, numlow, round(mean(as.numeric(cormatrix_stat[,2]),na.rm = TRUE),digits=2)))
    ...
    if (i > b) {
      ...
      for (pro in 1:numprobs) {
        if (numvalues[pro, i] >= mininst &&
          ...
          cormatrix_stat[num, 1] <- problemnames[pro]
          a <-
            cor.test(valmatrix[[pro]][[i]]+valmatrix[[pro]][[b]], valmatrix[[pro]]$CPU_time, method = "spearman")
          cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
          num <- num + 1
        }
      }
        ...
        if (b > c) {
          ...
          for (pro in 1:numprobs) {
            if (numvalues[pro, i] >= mininst &&
              ...
              cormatrix_stat[num, 1] <- problemnames[pro]
              a <-
                cor.test(valmatrix[[pro]][[i]]+valmatrix[[pro]][[b]]+valmatrix[[pro]][[c]], valmatrix[[pro]]$CPU_time, method = "spearman")
...
for (pro in unique(stat[, 1])) {
  ...
  if (lenst >= mininst) {
    a <- cor.test(st$numAtomsHead, st$CPU_time, method = "spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
numhigh <- 0
numlow <- 0
for (n in 1:nrow(cormatrix_stat)) {
  if (!is.na(cormatrix_stat[n, 2])) {
    if (abs(as.numeric(cormatrix_stat[n, 2])) >= 0.7) {
      numhigh <- numhigh + 1
    }
    if (abs(as.numeric(cormatrix_stat[n, 2])) <= 0.2) {
      numlow <- numlow + 1
    }
  }
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeRuleHead + st$numRules, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeAnswerSet + st$maxSizeHeadNegBodyRule, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  if (lenst >= mininst) {
    a <-
      cor.test(st$numNonHornClauses + st$maxSizeHeadNegBodyRule, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  if (lenst >= mininst) {
    a <-
      cor.test(
        st$maxSizeAnswerSet + st$numAtomsPosBody + st$maxNumVarOccHeadNegBody + st$numRules +
          st$numAtomsNegBody,
        st$CPU_time, method="spearman"
      )
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
...
for (pro in unique(stat[, 1])) {
  ...
  if (lenst >= mininst) {
    a <-
      cor.test(st$numAtomsPosBody + st$numNonHornClauses + st$numRules + st$numAtomsNegBody,
               st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$numRules + st$numAtomsNegBody, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  if (lenst >= mininst) {
    a <-
      cor.test(st$maxSizeHeadNegBodyRule + st$numNonHornClauses, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$numRules, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeAnswerSet, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeNCon + st$maxNumAtomOcc + st$numAtomsPosBody + st$numConstraints + st$maxSizeNegBodyCon + st$numAtomOccC, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeAnswerSet + st$maxSizeNegBodyRule + st$maxSizePosBodyCon + st$maxSizeNegBodyCon, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeAnswerSet + st$maxSizePosBodyNCon + st$numAtomsPosBody + st$numConstraints + st$maxSizeNegBodyCon + st$numAtomOccC, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeAnswerSet + st$maxSizePosBodyNCon + st$numNonHornClauses + st$maxNumAtomOcc + st$numAtomsPosBody + st$numRules, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeAnswerSet + st$maxSizePosBodyNCon + st$numNonHornClauses + st$maxNumVarOccHeadNegBody + st$numConstraints + st$maxSizeNegBodyCon + st$numAtomOccC, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizePosBodyNCon + st$numNonHornClauses + st$maxNumAtomOcc + st$numConstraints + st$maxSizeNegBodyCon + st$numAtomOccC, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
...
for (pro in unique(stat[, 1])) {
  ...
  cormatrix_stat[num, 1] <- pro
  if (lenst >= mininst) {
    a <- cor.test(st$maxSizeAnswerSet + st$maxSizePosBodyNCon + st$numNonHornClauses + st$maxNumAtomOcc + st$numRules + st$maxSizeNegBodyCon, st$CPU_time, method="spearman")
    cormatrix_stat[num, 2] <- round(a$estimate, digits = 2)
  }
  num <- num + 1
}
