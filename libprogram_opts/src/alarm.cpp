...
// we need a separate thread for timeout management.
    ...
    static struct LockInit {
        ...
        ~LockInit() {
            DeleteCriticalSection(AlarmThread::signalLock_s);
            delete AlarmThread::signalLock_s;
        }
    } lock_s;
...
unsigned long initMainThread() {
    ...
    if (AlarmThread::mainHandle_s == INVALID_HANDLE_VALUE) {
        if (DuplicateHandle(GetCurrentProcess(), GetCurrentThread(), GetCurrentProcess(), &AlarmThread::mainHandle_s, 0,
