...
///////////////////////////////////////////////////////////////////////////////
    Value::Value(byte_t flagSet, State initial)
            : state_(static_cast<byte_t>(initial)), flags_(flagSet), descFlag_(0), optAlias_(0) {
        desc_.value = 0;
    }
...
///////////////////////////////////////////////////////////////////////////////
    Option::Option(const string &longName, char alias, const char *desc, Value *v)
            : name_(longName), description_(desc ? desc : ""), value_(v) {
        assert(v);
            ...
            void assign(const ParsedValues &p) {
                ...
                for (ParsedValues::iterator end = p.end(); it != end; ++it) {
                    ...
                    if (int ret = self->assign(o, it->second)) {
                        throw ValueError(p.ctx ? p.ctx->caption() : "", static_cast<ValueError::Type>(ret - 1),
                                         o.name(), it->second);
                    }
                }
            }
...
///////////////////////////////////////////////////////////////////////////////    
        ...
        public:
            enum OptionType {
                short_opt, long_opt, end_opt, no_opt
            };
            CommandLineParser(ParseContext &ctx, unsigned f)
                    : OptionParser(ctx), flags(f) { }
            ...
            void doParse() {
                ...
                while ((curr = next()) != 0 && !breakEarly) {
                    switch (getOptionType(curr)) {
                        case short_opt:
                            if (handleShortOpt(curr + 1)) curr = 0;
                            break;
                        case long_opt:
                            if (handleLongOpt(curr + 2)) curr = 0;
                            break;
                        case end_opt:
                            curr = 0;
                            breakEarly = true;
        ...
        public:
            ArgvParser(ParseContext &ctx, int startPos, char **argv, unsigned flags)
                    : CommandLineParser(ctx, flags), currentArg_(0), argPos_(startPos), argv_(argv) {
            }
        ...
        public:
            CommandStringParser(const char *cmd, ParseContext &ctx, unsigned flags)
                    : CommandLineParser(ctx, flags), cmd_(cmd ? cmd : "") {
                tok_.reserve(80);
            }
        private:
            const char *next() {
...
///////////////////////////////////////////////////////////////////////////////    
        ...
        public:
            DefaultContext(const OptionContext &o, bool allowUnreg, PosOption po)
                    : posOpt(po), parsed(o), eMask(2u + unsigned(!allowUnreg)) { }
...
///////////////////////////////////////////////////////////////////////////////    
    ...
    static std::string format(SyntaxError::Type t, const std::string &key) {
        ...
        ret += quote(key);
        switch (t) {
            case SyntaxError::missing_value:
                ret += " requires a value!";
                break;
            case SyntaxError::extra_value:
                ret += " does not take a value!";
                break;
            case SyntaxError::invalid_format:
                ret += " unrecognized line!";
                break;
            default:
                ret += " unknown syntax!";
        };
    ...
    static std::string format(ContextError::Type t, const std::string &ctx, const std::string &key,
        ...
        switch (t) {
            case ContextError::duplicate_option:
                ret += "duplicate option: ";
                break;
            case ContextError::unknown_option:
                ret += "unknown option: ";
                break;
            case ContextError::ambiguous_option:
                ret += "ambiguous option: ";
                break;
            case ContextError::unknown_group:
                ret += "unknown group: ";
                break;
            default:
                ret += "unknown error in: ";
        };
        ret += quote(key);
    ...
    static std::string format(ValueError::Type t, const std::string &ctx, const std::string &key,
        ...
        switch (t) {
            case ValueError::multiple_occurences:
                ret += "multiple occurences: ";
                break;
