...
namespace bk_lib {
    ...
    static bool parseSigned(const char *&x, long long &out, long long sMin, long long sMax) {
        ...
        if ((std::strncmp(x, "imax", 4) == 0 && (out = sMax) != 0) ||
            (std::strncmp(x, "imin", 4) == 0 && (out = sMin) != 0)) {
            x += 4;
            return true;
        }
        const char *safe = x;
    ...
    static bool parseUnsigned(const char *&x, unsigned long long &out, unsigned long long uMax) {
        ...
        std::size_t len = 4;
        if (std::strncmp(x, "imax", len) == 0 || std::strncmp(x, "umax", len) == 0 ||
            std::strncmp(x, "-1", len = 2) == 0) {
            out = *x != 'i' ? uMax : uMax >> 1;
            x += len;
            return true;
        }
    ...
    int xconvert(const char *x, bool &out, const char **errPos, int) {
        ...
        else if (strncmp(x, "no", 2) == 0) {
            out = false;
            x += 2;
        }
        else if (strncmp(x, "on", 2) == 0) {
            out = true;
            x += 2;
        }
        else if (strncmp(x, "yes", 3) == 0) {
            out = true;
            x += 3;
        }
        else if (strncmp(x, "off", 3) == 0) {
            out = false;
            x += 3;
        }
        else if (strncmp(x, "true", 4) == 0) {
            out = true;
            x += 4;
        }
        else if (strncmp(x, "false", 5) == 0) {
            out = false;
            x += 5;
        }
    ...
    string &xconvert(string &out, unsigned int n) {
        return n != static_cast<unsigned int>(-1) ? xconvert(out, static_cast<unsigned long>(n)) : out.append("umax");
    }
