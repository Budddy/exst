...
/////////////////////////////////////////////////////////////////////////////////////////
    namespace Cli {
        ClaspAppOptions::ClaspAppOptions() : outf(0), ifs(' '), hideAux(false), onlyPre(false), printPort(false),
                                             outLbd(Activity::MAX_LBD), inLbd(Activity::MAX_LBD) {
        ...
        void ClaspAppOptions::initOptions(ProgramOptions::OptionContext &root) {
            ...
            basic.addOptions()
                    ...
                    ("lemma-out-lbd,@1", notify(this, &ClaspAppOptions::mappedOpts)->arg("<n>"),
                     "Only write lemmas with lbd <= %A")
                    ("lemma-in,@1", storeTo(lemmaIn)->arg("<file>"), "Read additional lemmas from %A")
                    ("lemma-in-lbd,@1", notify(this, &ClaspAppOptions::mappedOpts)->arg("<n>"),
                     "Initialize lbd of additional lemmas to <n>")
...
/////////////////////////////////////////////////////////////////////////////////////////
        ...
        void ClaspAppBase::validateOptions(const ProgramOptions::OptionContext &,
            ...
            if (!app.lemmaOut.empty() && !isStdOut(app.lemmaOut)) {
                if (std::find(app.input.begin(), app.input.end(), app.lemmaOut) != app.input.end() ||
                    app.lemmaIn == app.lemmaOut) {
        ...
        void ClaspAppBase::run() {
            if (out_.get()) {
                out_->run(getName(), getVersion(), &claspAppOpts_.input[0],
                          &claspAppOpts_.input[0] + claspAppOpts_.input.size());
            }
            try { run(*clasp_); }
            catch (...) {
                try {
        ...
        void ClaspAppBase::readLemmas(SharedContext &ctx) {
            ...
            std::istream &file = isStdIn(claspAppOpts_.lemmaIn) ? std::cin : (fileStream.open(
                    claspAppOpts_.lemmaIn.c_str()), fileStream);
            Solver &s = *ctx.master();
        ...
        void ClaspAppBase::writeNonHcfs(const SharedDependencyGraph &graph) const {
            ...
            char buf[10];
            std::ofstream file;
            for (SharedDependencyGraph::NonHcfIter it = graph.nonHcfBegin(), end = graph.nonHcfEnd();
                 it != end; ++it, ++scc) {
                snprintf(buf, 10, ".%u", scc);
                std::string n = claspAppOpts_.hccOut + buf;
                if (!isStdOut(claspAppOpts_.hccOut) &&
                    (file.open(n.c_str()), !file.is_open())) { throw std::runtime_error("Could not open hcc file!\n"); }
...
// Creates output object suitable for given input format
        ...
        void ClaspAppBase::run(ClaspFacade &clasp) {
            ...
            while (prg.parseProgram(input) && handlePostGroundOptions(prg)) {
                ...
                if (!inc || clasp.result().interrupted() || !input.skipWhite() || *input != '9' ||
                    !clasp.update().ok()) { break; }
