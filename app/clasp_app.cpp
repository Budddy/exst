...
namespace Clasp
{
    ...
    inline bool isStdIn(const std::string &in)
    {
        return in == "-" || in == "stdin";
    }

    inline bool isStdOut(const std::string &out)
    {
    ...
    namespace Cli
    {
        ClaspAppOptions::ClaspAppOptions() :
                outf(0), ifs(' '), hideAux(false), onlyPre(false), printPort(false), outLbd(Activity::MAX_LBD), inLbd(Activity::MAX_LBD)
        {
        ...
        void ClaspAppOptions::initOptions(ProgramOptions::OptionContext &root)
        {
            ...
            basic.addOptions()("quiet,q", notify(this, &ClaspAppOptions::mappedOpts)->implicit("2,2,2")->arg("<levels>"),
                               "Configure printing of models, costs, and calls\n"
                                       ...
                                       "        <call>: print {0=all|1=last|2=no} call steps      [2]")("pre", flag(onlyPre), "Run preprocessing and exit")(
                    "print-portfolio", flag(printPort), "Print default portfolio and exit")("outf,@1", storeTo(outf)->arg("<n>"),
                                                                                            "Use {0=default|1=competition|2=JSON|3=no} output")("out-atomf,@1",
                                                                                                                                                storeTo(outAtom),
                                                                                                                                                "Set atom format string (<Pre>?%%s<Post>?)")(
                    "out-ifs,@1", notify(this, &ClaspAppOptions::mappedOpts), "Set internal field separator")("out-hide-aux,@1", flag(hideAux),
                                                                                                              "Hide auxiliary atoms in answers")("lemma-out,@1",
                                                                                                                                                 storeTo(lemmaOut)->arg(
                                                                                                                                                         "<file>"),
                                                                                                                                                 "Write learnt lemmas to %A on exit")(
                    "lemma-out-lbd,@1", notify(this, &ClaspAppOptions::mappedOpts)->arg("<n>"), "Only write lemmas with lbd <= %A")("lemma-in,@1",
                                                                                                                                    storeTo(lemmaIn)->arg(
                                                                                                                                            "<file>"),
                                                                                                                                    "Read additional lemmas from %A")(
                    "lemma-in-lbd,@1", notify(this, &ClaspAppOptions::mappedOpts)->arg("<n>"), "Initialize lbd of additional lemmas to <n>")("hcc-out,@1",
                                                                                                                                             storeTo(hccOut)->arg(
                                                                                                                                                     "<file>"),
                                                                                                                                             "Write non-hcf programs to %A.#scc")(
        ...
        bool ClaspAppOptions::mappedOpts(ClaspAppOptions *this_, const std::string &name, const std::string &value)
        {
            ...
            if (name == "quiet")
            {
                ...
                {
                    this_->quiet[i] = static_cast<uint8>(q[i]);
                }
                return parsed && *err == 0;
            ...
            {
                if (name == "lemma-out-lbd")
                {
                    this_->outLbd = (uint8) x;
                    return true;
                } else if (name == "lemma-in-lbd")
                {
                    this_->inLbd = (uint8) x;
                    return true;
                }
        ...
        {
            if (quiet[1] == static_cast<uint8>(UCHAR_MAX))
            {
                quiet[1] = quiet[0];
            }
            return true;
        ...
        void ClaspAppBase::validateOptions(const ProgramOptions::OptionContext &, const ProgramOptions::ParsedOptions &parsed,
        ...
        void ClaspAppBase::setup()
        {
            ...
            if (!claspAppOpts_.onlyPre)
            {
                ...
                if (out_.get() && out_->verbosity() < (uint32) verb)
                {
                    verb = (Event::Verbosity) out_->verbosity();
                }
        ...
        void ClaspAppBase::shutdown()
        {
            if (clasp_.get())
            {
                ...
                if (!claspAppOpts_.lemmaOut.empty())
                {
                    ...
                    Constraint_t::Set x;
                    x.addSet(Constraint_t::learnt_conflict);
        ...
        void ClaspAppBase::run()
        {
            if (out_.get())
            {
                out_->run(getName(), getVersion(), &claspAppOpts_.input[0], &claspAppOpts_.input[0] + claspAppOpts_.input.size());
            }
            try
            {
                run(*clasp_);
        ...
        {
            exst::ExtendedParameters::getInstance().setSymbolTable(s.symbolTable());
            exst::ExtendedParameters::getInstance().addAnswerSet(m);
            bool ret = true;
        ...
        {
            printf("# exst %s configuration file\n"
                           ...
                           "# and   <cmd>  is a command-line string of exst options in long-format, e.g.\n"
        ...
        void ClaspAppBase::readLemmas(SharedContext &ctx)
        {
            ...
            for (ClauseCreator clause(&s); file && ok;)
            {
                ...
                {
                    const char *m = file.get() == 'p' ? " cnf" : " exst";
                    while (file.get() == *m)
                    {
                        ++m;
                    }
                ...
                while ((file >> x))
                {
                    ...
                    if (!s.validVar(x.var()))
                    {
                        throw std::runtime_error("Bad variable in file: " + claspAppOpts_.lemmaIn);
                    }
                    if (!elim)
                    {
                        clause.add(x);
                ...
                if (x.var() != 0)
                {
                    throw std::runtime_error("Unrecognized format: " + claspAppOpts_.lemmaIn);
                }
            }
            if (ok && !file.eof())
            {
                throw std::runtime_error("Error reading file: " + claspAppOpts_.lemmaIn);
            }
            s.simplify();
        }
        ...
        void ClaspAppBase::writeNonHcfs(const SharedDependencyGraph &graph) const
        {
            ...
            for (SharedDependencyGraph::NonHcfIter it = graph.nonHcfBegin(), end = graph.nonHcfEnd(); it != end; ++it, ++scc)
            {
                ...
                if (!isStdOut(claspAppOpts_.hccOut) && (file.open(n.c_str()), !file.is_open()))
                {
                    throw std::runtime_error("Could not open hcc file!\n");
                }
        ...
        std::istream &ClaspAppBase::getStream()
        {
            ...
            } else
            {
                ...
                if (!file)
                {
                    throw std::runtime_error("Can not read from '" + input[0] + "'");
                }
                return file;
            }
        }
        ...
        Output *ClaspAppBase::createOutput(ProblemType f)
        {
            ...
            if (claspAppOpts_.outf != ClaspAppOptions::out_json || claspAppOpts_.onlyPre)
            {
                ...
                if (f == Problem_t::SAT)
                {
                    outFormat = TextOutput::format_sat09;
                } else if (f == Problem_t::PB)
                {
                    outFormat = TextOutput::format_pb09;
        ...
        {
            if (!claspAppOpts_.onlyPre || prg.type() != Problem_t::ASP)
            {
                return true;
            }
            if (prg.endProgram())
            {
                static_cast<Asp::LogicProgram &>(prg).write(std::cout);
            } else
            {
                std::cout << "0\n0\nB+\n1\n0\nB-\n1\n0\n0\n";
            }
        ...
        {
            if (!claspAppOpts_.lemmaIn.empty() && clasp.step() == 0)
            {
                readLemmas(clasp.ctx);
            }
            if (!claspAppOpts_.hccOut.empty() && clasp.ctx.sccGraph.get())
            {
        ...
        void ClaspAppBase::run(ClaspFacade &clasp)
        {
            ...
            if (inc)
            {
                inc = clasp.enableProgramUpdates();
            } else
            {
            ...
            while (prg.parseProgram(input) && handlePostGroundOptions(prg))
            {
                ...
                if (!inc || clasp.result().interrupted() || !input.skipWhite() || *input != '9' || !clasp.update().ok())
                {
                    break;
                }
        ...
        WriteLemmas::~WriteLemmas()
        {
            detach();
        }

        void WriteLemmas::detach()
        {
            if (ctx_)
            {
                ctx_ = 0;
